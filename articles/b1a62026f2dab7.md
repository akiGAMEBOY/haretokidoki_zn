...
```powershell:
PS D:\Downloads> $test = (Get-ChildItem function:Test-FileLocked).Definition
PS D:\Downloads>
PS D:\Downloads> $test.GetType().FullName
System.String
...
function Get-FunctionDefinition {
    param (
        ...
        [System.String]$NewlineCode="`r`n",
        [System.Boolean]$MarkdownMode=$False
    )

    # スクリプトブロック内の改行コードを「LF」で設定
    $scriptblockNewlineCode = "`n"
    $plainDefinition = (((Get-Command $FunctionName).ScriptBlock.ToString() -Split $scriptblockNewlineCode | Select-Object -Skip 1) -Join $NewlineCode)

    $functionDefinition = ''
    if ($MarkdownMode) {
        $functionDefinition = "``````powershell$($NewlineCode)"
        $functionDefinition += "function $FunctionName {$($NewlineCode)"
        $functionDefinition += $plainDefinition
        $functionDefinition += "}$($NewlineCode)"
        $functionDefinition += "``````$($NewlineCode)"
    }
    else {
        $functionDefinition = "function $FunctionName {$($NewlineCode)"

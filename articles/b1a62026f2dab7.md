...
- コマンドレットでFunction定義を確認する方法
    - Get-ChildItemコマンドレットを使用した2つの方法
    - Get-Commandコマンドレットを使用した2つの方法
- 自作したFunctionでFunction定義を確認する方法
    定義を確認する以外に **改行コードの指定** と **Markdownのコードブロック形式での出力** を可能とした。
...
````powershell:その他の引数「改行コード: LF 、 Markdown形式出力: オン」で自作Functionを実行
...
上記だと改行コードは判断できないと思いますが、Markdown形式（```````powershell` ではじまり、````````）で終わっていることを確認できました。

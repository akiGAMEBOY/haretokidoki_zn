...
`winget list` の実行結果で確認できますが、一括アップデート可能なアプリはソースが 「`winget`」か「`msstore`」となっているアプリのみが対象となります。
ソースが「空欄」のアプリについては対象外となりアップデート不可です。

そこで今回、私の環境で導入されているアプリの中でソースが「空欄」のアプリを対象とし、アプリ名で `winget serach` コマンド（もしくは、`winget show` コマンド）で検索。
...
|【有無】<br>|【コメント】<br>|【winget search/showの結果】<br>　名前|<br>　ID|<br>　バージョン|<br>　ソース|【winget listの結果】<br>　名前|<br>　ID|<br>　バージョン|<br>　利用可能|<br>　ソース|
|:---:|---|---|---|---|---|---|---|---|---|---|
...
wingetでソース「`winget`」か「`msstore`」のアプリをインストールするコマンドはこちら。
...
なお、これら大量のコマンドを作成した方法は、リストアップした表を元にExcelの[CONCAT関数](https://support.microsoft.com/ja-jp/office/concat-関数-9b1a9a3f-94ff-41af-9736-694cbd6b4ca2)でコマンドを作成、
さらにPowerShellで一括実行できるよう[TEXTJOIN関数](https://support.microsoft.com/ja-jp/office/textjoin-関数-357b449a-ec91-49d0-80c3-0e8fc845691c)でセミコロン（`;`）を入れて作成しました。
...
```powershell:私の環境でwingetアプリ（winget or msstore）をインストールした際のコマンド
...
- 既存のMSIX系アプリをMicrosoft Store製のアプリに入れ替える事で、CLIやGUIで一括アップデートできる環境を構築できた！

...
winget コマンドを使用すると、Windows システムに導入しているアプリを一括アップデート（もしくはアップグレード）が可能。

ただ、winget list の ソースで 「winget」or「msstore」となっているアプリだけが、
winget upgrade コマンドで一括アップデートできる仕様のようで自身の環境にあるソースが「空欄」のアプリについては、
アップデート対象にならないもよう。

そこで、ソースが「空欄」となっているアプリ名を使い winget search コマンド でアプリを検索。
ソース「winget」か「msstore」で配信されている場合は、アプリを入れ替えて少しでも一括アップデート対象にしようという計画。

## アップグレード対象外だったアプリ一覧

winget list コマンドの実行結果のうち、ソース が（空欄）だったアプリ。
かつ、winget search コマンドでアプリ名を検索した結果、ソース が winget or msstore で存在したアプリ。

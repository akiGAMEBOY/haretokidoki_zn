---
title: "wingetで一括アップデートするために既存のアプリをMSSTORE経由で再インストールしてみた"
emoji: "🐙"
...
また、wingetではコマンド（`winget upgrade`）で一括アップデートも可能となります。
...
この作業を行い、既存のMSIX系アプリを一括アップデート可能とし、手動アップデートの作業効率を上げましょう。
...
不具合により `winget search` では検索結果に表示されないMicrosoft Store製のアプリが存在します。
この結果に表示されないアプリは、`winget show` コマンドを使用する事で`ID`など詳細な情報を確認可能です。

2つのコマンドで検索し対象アプリをリストアップする必要があり少し作業が面倒ですね。
...
:::details ソース が「空欄」だったアプリすべての一覧表 < クリックで折りたたみが開く >

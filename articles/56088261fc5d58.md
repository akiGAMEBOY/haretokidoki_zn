...
個人の環境により変化する内容となりますので、各々の環境でチェックしてください。
...
:::message 参考情報：複数IDを検索キーにwinget showの結果を抽出し表示するFunction

**winget show コマンド結果の内、必要な情報だけを表示可能**

[こちらの記事](https://zenn.dev/haretokidoki/articles/d2c98229b3ca95)で紹介。
`winget list` のソースが空欄だったアプリのIDをリストアップする事により、winget show で必要な情報となる下記3点を取得できます。

- アプリ名
- ID
- ソース

検索キーがIDではなくアプリ名にしたい場合は、コードを変更してご対応ください。

---
title: "wingetで一括アップデートするために既存アプリをMSStore版アプリとして再インストールしてみた"
emoji: "🏪"
...
[こちら](https://zenn.dev/haretokidoki/articles/aea5b45679d966#コマンドで「python」をインストール)の記事内でも一部紹介していますが、
wingetコマンド（MSStoreでは、[アプリ インストーラー](https://www.microsoft.com/store/productId/9NBLGGH4NNS1?ocid=pdpshare)）でもアプリを導入可能です。
また、wingetではCLI（`winget upgrade`）で一括アップデートが可能。それとMicrosoft Store版のアプリとして導入し[管理ソフト](https://www.microsoft.com/store/productId/9WZDNCRFJBMP?ocid=pdpshare)を使用する事でGUIのアップデートも可能となります。

`winget list` の実行結果でも確認できますが、一括アップデート可能なアプリはソースが 「`winget`」か「`msstore`」となっているアプリのみが対象で、ソースが「空欄」のアプリについては対象外です。

そこで今回、私の環境で導入されているアプリの中でソースが「空欄」のアプリを対象にアプリ名を引数に `winget serach` コマンド（もしくは、`winget show` コマンド）で検索。
検索した結果、ソース「`winget`」か「`msstore`」でも配信されているアプリであれば、既存のアプリをアンインストールし、
いずれかのソースのアプリとして再インストールしました。

既存のMSIX系アプリをMicrosoft Store製のアプリに入れ替える事で、CLIやGUIで一括アップデートできるような環境を構築しましょう。
...
この `winget search` に表示されないアプリは、`winget show` コマンドを使う事でインストールに必要な情報（IDなど）を取得できます。

2つのコマンドで検索し対象アプリをリストアップする必要がある為、少し面倒ですね。

|**有無**|**コメント**|winget search/show 確認結果||||winget list コマンド結果|||||
|:---:|:---:|---|---|---|---|---|---|---|---|---|
...
MS Store版のアプリをインストールするコマンドはこちら。
...
一つひとつのアプリを対象にコマンドを実行していると、時間がかかってしまう為、

1. 対象の既存アプリをすべてアンインストール
1. Microsoft Store版のアプリをインストール

という流れで作業しました。

なお、大量のコマンドを作成した方法は、リストアップした表を元にExcel関数のCONCAT関数でコマンドを作成、
PowerShellで一括実行できるようTEXTJOIN関数でセミコロン（`;`）を入れて作成しました。

```powershell:私の環境で既存アプリをアンインストールした際のコマンド
...
```powershell:私の環境でMSSTORE版アプリをインストールした際のコマンド
...
- 既存アプリをMSStoreアプリとして再インストールすることでGUIやCLIでも一括アップデート可能となる
    CLIは、`winget upgrade` コマンドを使用。GUIは、[Microsoft Store](https://www.microsoft.com/store/productId/9WZDNCRFJBMP?ocid=pdpshare) を使用。
- Microsoft Storeに配信されているかアプリ名で検索するが、アプリによって `winget list` で表示されないアプリあり
    `winget show` コマンドを使用する事で検索可能。
- 今回の作業を行う事で手動で一括アップデートする作業の効率をあげることができた！

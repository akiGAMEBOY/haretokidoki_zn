---
title: "wingetで一括アップデートするために既存のアプリをMSStore版アプリとして再インストールしてみた"
emoji: "🐙"
...
また、wingetではCLI（`winget upgrade`）で一括アップデートが可能。それとMicrosoft Store版のアプリとして導入する事により、管理ソフトを使いGUIでもアップデート可能です。
...
:::details 補足情報：ソース が「空欄」だったアプリすべての一覧表 < クリックで折りたたみが開く >

参考までに私の環境下で、`winget list` コマンドの結果でソースが「空欄」だったアプリを対象に調べた結果を共有します。
...
Excel関数のCONCATで一つひとつのコマンドを作成し、複数行コマンドを一度に実行できるようTEXTJOINで区切り文字「;」を指定して文字列を結合。

```powershell:アンインストールした時のコマンド

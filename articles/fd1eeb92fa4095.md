...
---
## Target
* 実行環境はActive Directory ドメインに参加していないスタンドアローン同士のリモート接続

## 設定なしでそのままPSSessionコマンドを実行すると
エラーにより異常終了します。
```powershell
...
### 事前設定の切り戻し手順
前述した通り設定は簡単ですが、設定の切り戻しは手順が多く面倒です。
#### 接続先の設定戻し
...
1. PSRemotingの無効化
    ...
    **無効化後に複数の事後作業あり**
    ...
    ただし、完全にすべての設定が元に戻る訳ではないので無効化後に個別で手動作業が必要です。

    なぜ無効化の場合のみ手動で設定を戻す必要があるのかというと、PSRemoting以外でサービスを利用している可能性がある為、
    自動で全ての設定を元に戻す事はできないというのが理由です。
    :::
...
1. 無効化後の事後作業
    - HTTP WinRM リスナーを削除
        無効化時にメッセージでは“IP アドレスを使用して要求を受け付けるリスナー”とあり、わかり難いメッセージですが、
        “HTTP WinRM リスナー”の事を指しています。
        :::message
        **注意事項**
    ...
    - WinRM（Windows Remote Management）のサービスの停止とスタートアップの種類の変更
        1. 作業前の状態
            ...
            PS C:\WINDOWS\system32>
            ```
            :::details 参考：スタートアップの種類をコマンドで確認する場合（このブロックを選択すると折りたたみが開く）
            - 確認結果
                ```powershell
        ...
        1. 作業後の状態
            下記コマンドでは、実行中（Running）か停止中（Stopped）等のステータスしかわからない。
            スタートアップの種類を確認する場合は、GUIのサービス画面（`services.msc`）を確認する必要あり。
            ```powershell
            PS C:\WINDOWS\system32> Get-Service WinRM
    ...
    - ローカル ユーザーのリモート管理権限のオフ（スタンドアローン環境のみ実施）
        ...
        **この項目はスタンドアローン環境でのみ必要な作業**
        ...
        作業端末が **Active Directoryドメインの配下にある場合、この作業は不要** です。

        ![所属ドメインを確認する方法](https://zenn.dev/haretokidoki/articles/e295b7da5f8a21)
        :::
...
1. 接続先から抜ける
    ...
    PS C:\WINDOWS\system32>
    ```
### New-PSSession と Get-PSSession と Remove-PSSession

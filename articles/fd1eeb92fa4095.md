...
---
## Target
- Active Directory ドメインに参加していないスタンドアローン同士のリモート接続
...
1. PSRemothingの有効化
    ...
    ```
    - 参考情報
        Active Diretory ドメイン環境の場合における対応方法の記載もあり。
...
1. PSRemotingの無効化
    ...
    **無効化後に事後作業あり**
    ...
    `Disable-PSRemoting`コマンドでは、PSSessionのアクセス許可設定が拒否に戻ります。
    ただし、完全に元の設定に戻る訳ではないので無効化後に手動で作業が必要です。

    これらが手動で設定を戻す必要がある理由は、PSRemoting以外でサービスを利用している可能性がある為、
    自動的に設定を戻す事はできません。
    :::
    ...
    - 作業後の状態    
        ...
        ```
1. 無効化後の事後作業
    - IP アドレスを使用して要求を受け付けるリスナーを削除
        :::message
        **注意事項**

        `Disable-PSRemoting`のコマンド結果で表示されるメッセージの順番と異なるが、
        WinRMサービスが起動中にリスナー名を確認する必要がある為、順番を入れ替えて実施。
        :::
        1. 削除対象のリスナー名を取得
            ```powershell
        ...
        1. 対象のリスナーが削除された事を確認            
            ...
            PS C:\WINDOWS\system32>
            ```
    - WinRM（Windows Remote Management）のサービスの停止
        1. 作業前の状態
            ```powershell
            ...
            PS C:\WINDOWS\system32>
            ```
            :::details 参考：スタートアップの種類をコマンドで確認する場合
            - 確認結果
                ```powershell
    ...
    - ローカル ユーザーのリモート管理権限のオフ（スタンドアローン環境のみ実施）
        レジストリ―のLocalAccountTokenFilterPolicy の値を戻す
        :::message
        **この項目はスタンドアローン環境でのみ必要な作業**

        スタンドアローン環境においてPSRemotningを有効化した場合のみ、レジストリキーが追加されます。
        仮に作業端末が **Active Directoryドメインの配下にある場合、この作業は不要** です。
        :::
        ...
        1. 作業前の状態
            ...
            PS C:\WINDOWS\system32>
            ```
            上記の通り「 LocalAccountTokenFilterPolicy : 1 」である事を確認
...
:::details 設定を切り戻す手順（このブロックを選択すると折りたたみが開く）
- 接続元の設定戻し
    1. 作業前の状態
        ```powershell
        PS C:\WINDOWS\system32> Get-Item WSMan:\localhost\Client\Trustedhosts
    ...
    - 手動の設定戻し作業
        1. IP アドレスを使用して要求を受け付けるリスナーを削除
            :::message
            **注意事項**

            `Disable-PSRemoting`を実行後に表示されるメッセージの順番と異なるが、
            WinRMサービスが起動中にリスナー名を確認する必要がある為、この手順では1と2を逆転。
            :::
            ```powershell
            PS C:\WINDOWS\system32> dir wsman:\localhost\listener
        ...
        1. WinRM サービスを停止または無効
            ...
            *画像：サービス画面 - “Windows Remote Management (WS-Management)”が無効化となっている事*
            :::details コマンドでサービスの詳細なステータスを確認する方法
            ```powershell:自作コマンド部分のみ
            ...
            >> )
            PS C:\WINDOWS\system32>
            PS C:\WINDOWS\system32> $serviceData | select サービス名,スタートアップの種類,状態 | Where-Object {$_.サービス名 -eq 'WinRM'}
        ...
        1. WS-Management 通信用のファイアウォール例外を無効
            ...
            PS C:\WINDOWS\system32>
            ```
        1. LocalAccountTokenFilterPolicy の値を戻す（スタンドアローン環境のみ）
            :::message
            **この項目はスタンドアローン環境で必要な作業**

            作業対象の端末がスタンドアローン環境の場合、`Enable-PSRemoting`の実行時にレジストリキーが追加されます。
            仮に作業端末が**Active Directoryドメインの配下にある場合は、この作業は不要**です。
            :::
            ```powershell
            PS C:\WINDOWS\system32> Get-ItemProperty -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\policies\system -Name LocalAccountTokenFilterPolicy
...
- Enter-PSSession
    ...
    1. 接続先から抜ける
        ...
        ```
- Exit-PSSession
    リモートコンピューターと対話形式のセッションを終了する
- Get-PSSession
    ローカルおよびリモートコンピューター上のPowerShellセッションを取得する
- New-PSSession
    ローカルまたはリモートコンピューターへの永続的なコネクションを作成するコマンド
    New-PSSessionは、ローカルまたはリモートのコンピューターへの永続的なコネクションを作成するコマンドです。
    ...
    ```
- Receive-PSSession
    切断されたセッションのコマンド結果を取得する
- Remove-PSSession
    1つまたは複数のPowerShellセッションを閉じる。
- Export-PSSession
    他のセッションのコマンドをエクスポートし、PowerShellモジュールに保存する。
- Import-PSSession
    他のセッションのコマンドを現在のセッションにインポートする。

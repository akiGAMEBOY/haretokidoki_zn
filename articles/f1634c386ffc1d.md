...
CD内にあるファイル形式（ファイル名や拡張子）のチェックと、
ファイル内の文字列を検索し特定の文字列[^1]の有無をチェックするツール。
...
flowchart TB
    ...
    F --- G("手動でスクリプトを再開")
    G --- H["CD内データのファイル形式（ファイル名と拡張子）を判定"]
    H --- I["CD内データで特定の文字列を検索し有無を判定"]

---
title: "[PowerShell]CDドライブのファイル内、任意の文字列の有無を判定するツール（DVDやBlu-ray、外部HDDも可）"
emoji: "💪"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["powershell", "CDドライブ"]
published: false
---
...
## サンプルプログラムの紹介
サンプルプログラムのシナリオは、複数のCDを所持しておりCD内にはログファイルが書き込まれている。
そのログファイルの中身を調べ処理結果が正常終了（Status=Success）しているか確認するという想定で作成したツール。
https://github.com/akiGAMEBOY/PowerShell_CD-DetermineStringTool
### 仕様
PowerShellスクリプト（ps1ファイルとbatファイル）でCD内にあるファイル内容をチェックするツール。

CDドライブのドライブレター（EドライブやFドライブ等）は設定ファイルにより、
変更できるようにする。
CDトレイのオープンは自動的に処理するが、
CDメディアをCDトレイにセットとCDトレイを閉じる操作は手作業で実施。
```mermaid
flowchart TB

---
title: "Windows 未登録の拡張子とソフトウェアを関連付ける（紐づける）方法"
emoji: "🤝"
...
nupkgファイル自体はZIP形式のファイルであり、今回はWindows標準の圧縮・解凍機能[^1]と関連付け（紐づけ）しました。
簡易な内容ですが、その方法を紹介します。
...
エラーが発生してしまいます。
::::details PowerShell CLIで実行した際のエラー内容
```powershell:assoc実行した際のエラー

...
ハードウェアの性能、実行する処理、繰り返し回数などの環境により結果は大きく変化すると思いますが、
今回の1万回繰り返して計測してみた結果、下記3点は誤差の範囲でした。

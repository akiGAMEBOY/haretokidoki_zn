---
title: "標準出力があるコマンドを渡すと標準出力を無視するコマンドに変換するFunction"
emoji: "🚂"
...
戻り値があるFunction内で標準出力の伴うコマンドを実行する場合、
PowerShellの制約上、そのコマンドの出力を無視する必要があります。

※ 参考記事：[PowerShell 関数の return について - マイクロソフ党ブログ](https://microsoftou.com/powershell-function/#toc7)

PowerShellでFunctionを作成する際、わざわざ標準出力を無視するコマンドにキータイプやテキストエディターの正規表現で置き換えていましたが、コマンド量があると面倒に感じていました。
今回、コマンドが入っている文字列配列を渡すと、標準出力を無視するコマンドに変換するFunctionを作成してみました。
...
前述した通り、対象のコマンドが入る文字列配列を引数としてFunctionに渡すと、
それらの先頭に`[void](`、末尾に`)`を入れる事でvoidキャストを実現します。
...
これを戻り値のあるFunction内で実行する場合は、下記のようにvoidにキャストし出力を破棄（または無視）が必要となります。

```powershell:voidにキャストした場合
[void](New-Item -Path 'D:\Downloads\work_folder' -ItemType Directory)
```

### その他の出力を無視する方法

上記で紹介したvoidキャスト以外にも出力を無視、または破棄する方法があります。

voidキャストを含めて4つ方法とその特徴を紹介。

なお、それぞれの方法で標準出力は無視できるが、標準エラーは無視できません。
エラー出力も考慮する必要がある場合は、try-catchでエラー制御する方法や、
標準エラーも標準出力として渡す方法などが考えられますが、この記事では説明を割愛します。
...
環境やハードウェアの性能、実行内容、繰り返し回数によっても大きく変化すると思いますが、
今回の結果から下記3点は誤差の範囲。
...
**ハイプでOut-Nullに渡す方法**だけは、他の手法よりも**遅い**事がわかります。

今回、作成したFunctionで変換する方法は、先頭と末尾に文字列を追加する必要がある**一番変換が面倒なvoidにキャストする方法に変換**してみます。
...
第2と第3に引数を指定しないとvoidにキャストする方法で変換するようにしました。

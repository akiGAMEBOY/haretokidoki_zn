...
:::details 引数 `-ProcessNames` を空で渡してデフォルトのブラウザ以外でも使えるようにしたい場合｜2026.07.17 追記

### 引数 `-ProcessNames` を空で渡してデフォルトのブラウザ以外でも使えるようにしたい場合｜2026.07.17 追記

`-ProcessNames "@()"` で 0 件になるときの原因と対処を紹介。

Windows バッチファイルから PowerShell スクリプトを呼び出すとき、  
「`-ProcessNames` を空配列で渡して、プロセスの絞り込みを無効化したい」と考えて次のように書くと ...

```bat
powershell -NoProfile -ExecutionPolicy RemoteSigned -File "../Set-WindowClientSize.ps1" -Width %TARGET_WIDTH% -Height %TARGET_HEIGHT% -ProcessNames "@()"
```

この書き方だと対象プロセスが 0 件になってしまします。

#### 起きていること

一見すると `@()` は PowerShell の空配列に見えますが、`-File` でスクリプトを呼び出した場合、バッチファイルから渡した値は PowerShell の式としては評価されません。

つまり、PowerShell コンソール上で直接書けば `@()` は空配列となりますが、
`bat` から `powershell -File` に渡した引数では、そのまま PowerShell 式として評価されずに **文字列 `@()` が 1 件入った配列** のように扱われてしまいます。

その結果、スクリプト側では次のような解釈になります。

1. `ProcessNames` は未指定ではない
1. ただし中身は `@()` という文字列
1. その名前のプロセスを探しに行く
1. 一致するプロセスが存在しない
1. 結果として 0 件になる

#### なぜ `-ProcessNames` を省略するだけでは駄目なのか

もしスクリプトが以下のような動きをするなら:

- `-ProcessNames` 未指定時: ブラウザ系の既定値を使う
- `-ProcessNames` に空配列を指定した時: 絞り込みなし

単純に引数を消すだけでは未指定時のデフォルト動作が有効になるため「絞り込みなし」にはなりません。 

#### 対処方法

`bat` から本当に空配列を渡したい場合は、`-File` ではなく `-Command` を使って、`@()` を **PowerShell の式として評価させる** 必要があります。
...
powershell -NoProfile -ExecutionPolicy RemoteSigned -Command ^
  "& '../Set-WindowClientSize.ps1' -Width %TARGET_WIDTH% -Height %TARGET_HEIGHT% -ProcessNames @()"

pause
```

`-Command` を使うと、PowerShell はコマンド文字列全体を解釈します。  そのため `@()` が単なる文字列ではなく、**本物の空配列** として扱われます。

カスタマイズして「デフォルトのブラウザ以外でも使えるようにしたい」という場合は、この方法で運用しましょう！

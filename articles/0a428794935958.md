...
(Get-Command -Name "対象コマンド名" -ErrorAction SilentlyContinue) -and (Write-Output 'Success!') | Out-Null
```
```powershell:コマンドが存在しない場合に「 Error! 」を表示
-Not(Get-Command -Name "対象コマンド名" -ErrorAction SilentlyContinue) -and (Write-Error 'Error!') | Out-Null
```
...
PS C:\Users\XXXX>
```
- 参考情報：PSGalleryを"信頼されていない（`Untrusted`）"から"信頼できる（`Trusted`）"に変更する方法

...
## 共通処理のまとめ
:::message
私が普段、使用しているモジュール「mdlCommon.bas」から抜粋している為、
汎用的になっていない箇所があるかと思います。

利用される際はご自身の用途に合わせてカスタマイズし、ご対応ください。
:::
### Functionプロシージャ
#### 空文字チェック
```vbnet
...
' 定数
'   メイン
Public Const MAIN_RANGE As String = "D3,D4,D5,D6"
' 　エラー情報
Public Const ERROR_RANGE As String = "D9,D10"
' 　入力情報
Public Const INPUT_RANGE As String = "D13,D14,D15,D16"
' 　出力情報
Public Const OUTPUT_RANGE As String = "D19,D20"
' 　マスター
Public Const MASTER_RANGE As String = "D23,D24,D25"
' 　非表示するシート情報
Public Const HIDDEN_RANGE As String = "D28,D29"
'   シート名
    ...
    If Not (wsFmSheet Is Nothing) And _
       Not (wsToSheet Is Nothing) Then
        ...
        With wsToSheet
            ...
            lngToEn= .Cells(1, Columns.Count).End(xlToLeft).Column
...
Sub SubShowMessagebox(lngCode As Long, Optional strAppend As String = "")
    ...
    For lngCount = LBound(aryMessages, 1) To UBound(aryMessages, 1)
        If aryMessages(lngCount, 0) = CStr(lngCode) Then
...
'*-------------------------------------------------------------------------------
'* 戻り値　｜-
'* 引数　　｜lngCode：対象ラベルコード、[任意]strAppend：追加メッセージ
'********************************************************************************
Public Sub SubDisplayMessage(lngCode As Long, Optional strAppend As String = "")
    ...
    aryMessages(1, 1) = ""
    aryMessages(2, 0) = "211"
    aryMessages(2, 1) = "フォルダ作成完了。"
    aryMessages(3, 0) = "-1"
    aryMessages(3, 1) = "異常終了。"

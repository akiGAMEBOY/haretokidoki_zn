...
2024年6月現在、調べたかぎり「Markdown形式のテーブル から Excelファイル への変換」についてはオンラインWebツール や Pythonスクリプト を使用する必要がありそうです。

オンラインWebツールを使用する際、機密情報を含むデータのアップロードは危険です。
...
ただ、今回作成したPowerShellコードでは、Excelを取り扱うために専用のモジュールを事前に導入が必要となります。

また、そのモジュール「ImportExcel」をインストールする際は、管理者権限でコマンドレット「`Install-Module`」の実行が必要。

### 事前準備

前述したとおり、今回はPowerShellスクリプトでExcelファイルの制御が必要な為、モジュール「ImportExcel」をインストールします。
...
- 入力ファイル：テーブルのみ記述しているMarkdown形式ファイル
    ...
    ```markdown:inputMarkdownTable.md
...
function Get-MarkdownTable {
    ...
    foreach ($row in $inputContents) {
        # ヘッダーの項目をベースにデータを出力するため、項目数の比較は行わない
        $rowData = @()
        foreach ($header in $headers) {
            $cellValue = $row."$header"
            $rowData += $cellValue
        }
        $markdownTables += '| ' + ($rowData -join ' | ') + ' |' + "`r`n"
    }

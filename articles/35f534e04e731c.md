---
title: "“Markdownのテーブル形式”と“Excelファイル”を相互変換するPSスクリプト"
emoji: "🌐"
...
ただ、今回はPowerShellのコードでExcelを取り扱うため、事前にExcelが制御可能となるモジュール「ImportExcel」の導入が必要。
...
下記のコードを実行することで、モジュール「ImportExcel」が導入可能。
なお、繰り返しにはなりますが、このコードを実行するPowerShellウィンドウは“管理者として実行”で起動（**管理者権限**）してコードを実行します。
...
:::details 補足情報：サンプルのインプット／アウトプットのデータ

https://github.com/akiGAMEBOY/haretokidoki-zn_articles_35f534e04e731c

- 入力ファイル：テーブルのみ記述しているMarkdown形式ファイル

- 出力ファイル：上記のMarkdown形式ファイルで出力したExcelファイル
    outputExcel.xlsx
...
- 入力ファイル：A1からヘッダーA2からコンテンツを記述しているExcelファイル
    inputExcel.xlsx
- 出力ファイル：上記のMarkdown形式ファイルで出力したMarkdown形式ファイル
...
- 入出力の要件が細かくある場合はカスタマイズが必要
    とくにExcelファイルがからむ場合はヘッダーの位置や書き出し位置など必要に応じてコードの修正が必要

...
:::details $PSDebugContext のデータ型をチェックした時
...
Write-Host $PSDebugContext.GetType().FullName   # ← デバッグ中に上記の行で停止中にコマンドを実行して確認
                                                # 　 * ブレーク後に続行するとnullエラー（null 値の式ではメソッドを呼び出せません。）となる
```
...
:::details $PSSenderInfo のデータ型をチェックした時

...
PS C:\WINDOWS\system32>
```
:::details 便利情報：コマンドのプロパティを調べる「Get-Member」
...
| `$foreach` | 実行環境により変化<br>* 参考:調査時のエビデンス<br>* 初期起動時に確認するとエラー（nullだった） |
| `$HOME` | System.String |
...
```powershell:$Event と $Sender と $this のデータ型をチェックした時
...
:::details $EventArgs のデータ型をチェックした時
処理内容によって変化する。下記のサンプルコードでは、

- $EventArgsが
    [`System.IO.FileSystemEventArgs`]

```powershell:$EventArgs のデータ型をチェックした時
...
:::details $Event と $EventSubscriber 、 $Sender のデータ型をチェックした時
処理内容によって変化する。下記のサンプルコードでは、
...
```powershell:ファイル監視の実装部分。実行すると常駐プロセスとなる
...
```powershell:上記のファイル監視を実行し、監視場所のテキストファイルを更新した結果
PS C:\WINDOWS\system32>
no. 0
change type: Changed
...
:::details $foreach のデータ型をチェックした時
処理内容によって変化する。下記のサンプルコードでは、

- $foreachが
    [`System.Array+SZArrayEnumerator`]

```powershell:$foreach のデータ型をチェックした時
...
:::details $Matches のデータ型をチェックした時

- $Matchesが
    [`System.Collections.Hashtable`]

```powershell:$Matches のデータ型をチェックした時
# マッチした場合
...
:::details $PSCmdlet のデータ型をチェックした時
```powershell:$PSCmdlet のデータ型をチェックした時

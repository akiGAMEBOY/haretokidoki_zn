...
| `$?` | System.Boolean |
| `$^` | System.String |
| `$_（$PSItem）`<br>* パイプラインで渡ってくるオブジェクトにより変化 | `Get-Process \| ForEach-Object -Process { Write-Host $_.GetType().FullName }` だと `System.Diagnostics.Process`。 <br>* 初期起動時に確認するとエラー（nullだった） |
| `$args` | System.Object[] |

...
- ハイパーバイザ（仮想化OS）
    ...
    ![Oracle VM VirtualBox マネージャーのバージョン情報の画面によりバージョンが「7.0.6.r155176(Qt5.15.2)」である事を確認](https://storage.googleapis.com/zenn-user-upload/d2a871322131-20230622.png)
...
- 仮想ゲストOS
    ...
    ![Oracle VM VirtualBox マネージャーよりFreeBSD OSを立ち上げブートメニューが起動した様子](https://storage.googleapis.com/zenn-user-upload/c10a757ff861-20230623.png)
    *画像：FreeBSD ブートメニュー*
...
:::details その他 OSのサポート対象（このブロックを選択すると折りたたみが開く）
...
1. [公式サイトのダウンロードページ](https://www.freebsd.org/where/)より `FreeBSD 13.1-RELEASE` -> `Installer` -> **amd64** を選択
    ![FreeBSD OSの公式サイトのダウンロードページにある「amd64」のリンクがある場所](https://storage.googleapis.com/zenn-user-upload/53492ba9f606-20230623.png =800x)
...
1. 下記のisoファイルを選択しダウンロード
    - **FreeBSD-13.1-RELEASE-amd64-dvd1.iso**
        ...
        空き領域に余裕がない場合、CDのisoファイル（`FreeBSD-13.1-RELEASE-amd64-disc1.iso`）でも可。
...
1. 作成した仮想マシンを選択し、起動ボタンをクリック
    ...
    ![Oracle VM VirtualBox マネージャーの起動メニューでヘッドレス起動を選択する様子](https://storage.googleapis.com/zenn-user-upload/39d7c191577b-20230623.png)
    *画像：ヘッドレス起動を行う操作*
...
- VirtualBoxで設定可能なプロセッサー最大値 と 物理CPUのコア数（もしくはスレッド数）と差異あり
    ...
    そもそもなぜ、VirtualBox側でスレッド数を超える値が設定可能なのかという点は、よくわからなかった。
    - 参考情報
...
:::details なぜDNSの設定を行ったか（このブロックを選択すると折りたたみが開く）
...
設定したIPアドレスは、すべてコメント化された状態で「`nameserver 127.0.0.1`」のみが有効となっていた。
...
###### 参考情報：FreeBSD をインストールしたらpkgやporstnapを使おうとしてもError Fetchingとなる
...
:::details 参考情報：設定後の/etc/ssh/sshd_config - すべて（このブロックを選択すると折りたたみが開く）
...
root@dev-api:~ # vi /etc/ntp.conf
server ntp.nict.jp 👈 追加
root@dev-api:~ #
```

:::details 参考情報：設定後の/etc/ntp.conf - すべて（このブロックを選択すると折りたたみが開く）
...
1. FreeBSD セキュリティパッチのダウンロード
    ...
    :::details 参考情報：FreeBSD ダウンロード時のコマンド結果 - すべて（このブロックを選択すると折りたたみが開く）
...
:::details 参考情報：インストール時のコマンド結果 - すべて（このブロックを選択すると折りたたみが開く）
...
1. ダウンロードしたインストーラーが最新か判定（改ざんがない事を確認）
    ...
    **コマンド結果が「Installer corrupt」の場合**
    ...
    最初に**コマンドのハッシュ値が古くないか**という点を疑った方が良い。
...
1. 最新のComposerをインストール
    ...
    **バージョンを指定してインストールする場合は、オプションを指定**
    ...
    たとえばバージョンを`2.0.12`で指定しインストールしたい場合、
    `php composer-setup.php -V="2.0.12"`
    というコマンドで実現可能。

    :::details インストーラー「composer-setup.php」のヘルプ（このブロックを選択すると折りたたみが開く）
...
1. インストールコマンドの実行
    ...
    **（私の環境では）インストールコマンドを実行時にエラーが発生**
    ...
    ::::details エラーへの対処方法（このブロックを選択すると折りたたみが開く）
    - 事象
        エラー内容は上記に記載。下記は実際に表示された際のエラー画面。
        ![Compower経由でtwitteroauthをインストールしようとした結果、エラーが発生](https://storage.googleapis.com/zenn-user-upload/d96c1976be06-20230622.png)
    *画面: twitteroauthをComposer経由でインストールした際のエラー*
    - 原因
        ...
        **不確実な情報となる為、正しい情報でない可能性あり**
        ...
        - 契約中のISP事業者（プロバイダー）がIPv6に対応していない事
    ...
    - 対処方法
        ...
        1. `/usr/src/sys/amd64/conf/GENERIC` で IPv6の設定をコメントアウト
            ...
            :::details 参考情報：設定後の/usr/src/sys/amd64/conf/GENERIC - 全ぼう ※省略あり（このブロックを選択すると折りたたみが開く）
        ...
        1. カーネルの再構築
            ...
            :::details 参考情報：“make cleandepend”コマンド結果 - 全ぼう ※省略あり（このブロックを選択すると折りたたみが開く）
            ...
            :::
            :::details 参考情報：“make depend”コマンド結果 - 全ぼう ※省略あり（このブロックを選択すると折りたたみが開く）
            ...
            :::
            :::details 参考情報：“make”コマンド結果 - 全ぼう ※省略あり（このブロックを選択すると折りたたみが開く）
            ...
            :::
            :::details 参考情報：“make install”コマンド結果 - 全ぼう ※省略あり（このブロックを選択すると折りたたみが開く）
...
- Googleが提供しているIPv6対応有無のチェックツール
    ...
    ![私のインターネット環境を調べてみた結果、「問題は検出されませんでした」とメッセージあり。IPv6は対応されていないが、IPv6のウェブサイトは閲覧可能との事](https://storage.googleapis.com/zenn-user-upload/e39a9b5368cb-20230628.png)

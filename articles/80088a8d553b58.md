...
FreeBSDインストール直後、pkgコマンドのアップデートを実施したがエラーとなった。
調べた結果、**DNSの設定に問題があり**インターネットに接続できていなかった模様。
...
###### 対応方法
DVDのisoファイルを使用しインストールしていた際に入力した情報があったが、
全てコメントされた状態で「nameserver 127.0.0.1」のみ有効としていた。
Configファイルをコメントアウトまたはコメントする事により、
「nameserver（接続するDNSサーバー）」の切り替えを行った。
```command-line:修正前の/etc/resolv.conf
root@dev-api:~ # cat /etc/resolv.conf
# Generated by resolvconf
#nameserver 192.168.XXX.1 👈 デフォルトゲートウェイ（インストール時に設定）
...
nameserver XXX.XXX.XXX.XXX 👈 コメントアウト
nameserver XXX.XXX.XXX.XXX 👈 コメントアウト
...
```command-line 設定後の/etc/ssh/sshd_config（抜粋）
...
```command-line 設定後の/etc/ssh/sshd_config（すべて）
...
```command-line 設定後の/etc/ntp.conf（抜粋）
server ntp.nict.jp 👈 追加。公開NTPサーバーとなるNICT（国立研究開発法人情報通信研究機構）のNTPサーバーを設定。
root@dev-api:~ #
```
::::details 参考情報：設定後の/etc/ntp.conf（すべて）
```command-line 設定後の/etc/ntp.conf（すべて）
...
server ntp.nict.jp 👈 公開NTPサーバーを設定［国立研究開発法人情報通信研究機構（NICT）のNTPサーバー］
...
1. FreeBSD セキュリティパッチのダウンロード
    ```comannd-line ダウンロード時のコマンド結果（抜粋）
    ...
    ```comannd-line ダウンロード時のコマンド結果（すべて）
    root@dev-api:~ # freebsd-update fetch
    Looking up update.FreeBSD.org mirrors... 2 mirrors found.
    Fetching metadata signature for 13.1-RELEASE from update2.freebsd.org... done.
    Fetching metadata index... done.
...
### STEP3：PHPの導入
#### STEP3-1：インストール方法の検討
この記事では、インストール時に複雑な設定は不要な為、
簡潔である**pkgコマンド**によるインストールで実施。
- pkgとportsの違い
    - pkgは、コンパイル済みのバイナリデータを使用しインストールする方法。ダウンロードするデータ総容量は大きいが、簡単、かつ速くインストール可能。
    - portsは、ソースファイルのコンパイルやインストールを手動で実行する方法。作業手順は多いが、ダウンロードするデータ総容量が少なく、かつ細かくカスタマイズしインストール可能。
...
```comannd-line インストール時のコマンド結果（抜粋）
...
Proceed with this action? [y/N]: y 👈 「y」で応答
...
```comannd-line インストール時のコマンド結果（すべて）
root@dev-api:~ # pkg install php81-8.1.17
Updating FreeBSD repository catalogue...
FreeBSD repository is up to date.
All repositories are up to date.
...
### STEP4：Composerの導入
パッケージ（pkgコマンド）でも`php81-composer2-2.5.5`が配布されているが、公式サイトにあるphpコマンド使った[こちらの方法](https://getcomposer.org/download/)を参考に実施。
1. 一時的な作業用ディレクトリを作成
    ```command-line
...
1. ダウンロードしたインストーラーが最新か判定（改ざんがない事を確認）
    ...
    **コマンド結果が「Installer corrupt」の場合**
    ...
    最新ではないと判断されるとコマンド結果が「**Installer corrupt**」となり、
    カレントディレクトリにダウンロードされたインストーラー「composer-setup.php」も、
    PHPのファイル削除関数“unlink”で自動的に削除される。
    
    このコマンドでは、ハッシュ値（sha384）により最新のインストーラーか判断される。
    　
    2023年6月現在に公式サイトから入手したコマンドのハッシュ値は、
    「`55ce33d7678c5a611085589f1f3ddf8b 3c52d662cd01d4ba75c0ee0459970c2200a51f492d557530c71c15d8dba01eae`」
    となっているが、
    その後、インストーラーが更新され**新しいハッシュ値となっている可能性が高い**。
    　
    最新のハッシュ値を含むコマンドは、[こちらの公式サイト](https://getcomposer.org/download/)より入手可能。
    :::
    ```command-line コマンド結果が「Installer verified」の場合、最新のインストーラーと判断
    root@dev-api:/tmp/20230621 # 
    ...
    Installer verified 👈 最新のインストーラーと判断
    root@dev-api:/tmp/20230621 # 
    ```
1. 最新のComposerをインストール
    ...
    **バージョンを指定してインストールする場合**
    ...
    ```command-line 最新のComposerをインストール（2023年6月現在、2.5.8が最新）
    ...
    root@dev-api:/tmp/20230621 #
    ```
1. インストーラーの削除（PHP関数を使用）
    ```command-line
    root@dev-api:/tmp/20230621 #
    root@dev-api:/tmp/20230621 # php -r "unlink('composer-setup.php');"
    root@dev-api:/tmp/20230621 #
    ```
1. composerコマンドの移動（カレント→/usr/local/bin配下）
    ```command-line
    root@dev-api:/tmp/20230621 #
    root@dev-api:/tmp/20230621 # ls -l
    total 2816
...
1. インストールコマンドの実行
    ...
    *画面：twitteroauthをComposer経由でインストールした際のエラー*
    - 原因
        :::message
        **不正確な情報な為、正しい情報ではない可能性あり**

        しっかりと切り分けし分析した結果ではなく、IPv6を無効化してみた結果、
        正しく動いたという表現が正確な情報となります。
        :::
        pkgコマンドやfreebsd-updateコマンド、curlコマンドなどによる外部アクセスは可能だが、
        ComposerコマンドのみエラーとなりIPv6無効化で正常動作した為、
        Composerコマンドでは、おそらく**IPv6を優先し接続されるのでエラー**となるのだと思われる。
    - 対処方法
        [こちら](https://qiita.com/ip6/items/4bf9579acab48670e387)のIPv4の無効化を紹介している記事を参考にIPv6の設定を無効化した結果、
        正常にcomposerコマンドが動作した。
        1. /usr/src/sys/amd64/conf/GENERIC で IPv6の設定をコメント
            ```command-line
            #options 	INET6			# IPv6 communications protocols 👈 コメント
            ```
            ::::details 参考情報：設定後の/usr/src/sys/amd64/conf/GENERIC（すべて）
            ```command-line
        ...
        root@dev-api:/usr/src/sys/amd64/compile/GENERIC #
        ```
        ::::details 参考情報：`make cleandepend`コマンド結果［すべて（省略あり）］
        ```command-line
        root@dev-api:/usr/src/sys/amd64/compile/GENERIC # make cleandepend
        ...
        ```
        ::::
        ::::details 参考情報：`make depend`コマンド結果［すべて（省略あり）］
        ```command-line
        root@dev-api:/usr/src/sys/amd64/compile/GENERIC # make depend
        ...
        bhnd_pwrctl_if.m /usr/src/sys/dev/bhnd/cores/chipc/bhnd_chipc_if.m /usr/src/sys/dev/bhnd/cores/pmu/bhnd_pmu_if.m /usr/src/sys/dev/bhnd/bhndb/bhndb_if.m /usr/src/sys/dev/bhnd/bhndb/bhndb_bus_if.m /usr/src/sys/dev/bhnd/nvram/bhnd_nvram_if.m /usr/src/sys/dev/bhnd/bhnd_bus_if.m /usr/src/sys/dev/bhnd/bhnd_erom_if.m /usr/src/sys/dev/hid/hid_if.m /usr/src/sys/arm/arm/platform_if.m /usr/src/sys/arm/arm/platform_pl310_if.m /usr/src/sys/arm/allwinner/sunxi_dma_if.m /usr/src/sys/arm/ti/ti_gpio_if.m /usr/src/sys/arm/nvidia/tegra_soctherm_if.m /usr/src/sys/arm/nvidia/drm2/tegra_dc_if.m /usr/src/sys/arm/nvidia/drm2/tegra_drm_if.m /usr/src/sys/geom/part/g_part_if.m /usr/src/sys/geom/raid/g_raid_md_if.m /usr/src/sys/geom/raid/g_raid_tr_if.m" KERNBUILDDIR="/usr/src/sys/amd64/compile/GENERIC" SYSDIR="/usr/src/sys" MODULE_TIED=yes WITH_CTF="1" KCSAN_ENABLED="yes" COMPAT_FREEBSD32_ENABLED="yes" make  obj
        ===> aac (obj)
        ...
        ===> accf_data (obj)
        /usr/src/sys/amd64/compile/GENERIC/modules/usr/src/sys/modules/accf_data created for /usr/src/sys/modules/accf_data
        ===> accf_dns (obj)
        ...
        ../../../dev/ath/if_ath_tx.c:1048:26: warning: variable 'wh' set but not used [-Wunused-but-set-variable]
                struct ieee80211_frame *wh;
                                        ^
        ../../../dev/ath/if_ath_tx.c:1563:19: warning: variable 'ds' set but not used [-Wunused-but-set-variable]
                struct ath_desc *ds;
                                ^
        ../../../dev/ath/if_ath_tx.c:1915:6: warning: variable 'is_ampdu' set but not used [-Wunused-but-set-variable]
         ...
         ～～～ 省略 ～～～
                    ...
                    ^
        ../../../dev/ath/if_ath_tx.c:5432:29: warning: variable 'tap' set but not used [-Wunused-but-set-variable]
        ...
        bhnd/cores/pmu/bhnd_pmu_if.m /usr/src/sys/dev/bhnd/bhndb/bhndb_if.m /usr/src/sys/dev/bhnd/bhndb/bhndb_bus_if.m /usr/src/sys/dev/bhnd/nvram/bhnd_nvram_if.m /usr/src/sys/dev/bhnd/bhnd_bus_if.m /usr/src/sys/dev/bhnd/bhnd_erom_if.m /usr/src/sys/dev/hid/hid_if.m /usr/src/sys/arm/arm/platform_if.m /usr/src/sys/arm/arm/platform_pl310_if.m /usr/src/sys/arm/allwinner/sunxi_dma_if.m /usr/src/sys/arm/ti/ti_gpio_if.m /usr/src/sys/arm/nvidia/tegra_soctherm_if.m /usr/src/sys/arm/nvidia/drm2/tegra_dc_if.m /usr/src/sys/arm/nvidia/drm2/tegra_drm_if.m /usr/src/sys/geom/part/g_part_if.m /usr/src/sys/geom/raid/g_raid_md_if.m /usr/src/sys/geom/raid/g_raid_tr_if.m" KERNBUILDDIR="/usr/src/sys/amd64/compile/GENERIC" SYSDIR="/usr/src/sys" MODULE_TIED=yes WITH_CTF="1" KCSAN_ENABLED="yes" COMPAT_FREEBSD32_ENABLED="yes" make  obj
        ===> aac (obj)
        ===> aacraid (obj)
        ===> accf_data (obj)
        ===> accf_dns (obj)
        ...
        cc  -O2 -pipe -fno-common  -fno-strict-aliasing -Werror -D_KERNEL -DKLD_MODULE -DKLD_TIED -nostdinc   -DHAVE_KERNEL_OPTION_HEADERS -include /usr/src/sys/amd64/compile/GENERIC/opt_global.h -I. -I/usr/src/sys -I/usr/src/sys/contrib/ck/include -fno-common -g -fno-omit-frame-pointer -mno-omit-leaf-frame-pointer -fdebug-prefix-map=./machine=/usr/src/sys/amd64/include -fdebug-prefix-map=./x86=/usr/src/sys/x86/include -I/usr/src/sys/amd64/compile/GENERIC     -MD  -MF.depend.zutil.o -MTzutil.o -mcmodel=kernel -mno-red-zone -mno-mmx -mno-sse -msoft-float  -fno-asynchronous-unwind-tables -ffreestanding -fwrapv -fstack-protector -Wall -Wredundant-decls -Wnested-externs -Wstrict-prototypes -Wmissing-prototypes -Wpointer-arith -Wcast-qual -Wundef -Wno-pointer-sign -D__printf__=__freebsd_kprintf__ -Wmissing-include-dirs -fdiagnostics-show-option -Wno-unknown-pragmas -Wno-error=tautological-compare -Wno-error=empty-body -Wno-error=parentheses-equality -Wno-error=unused-function -Wno-error=pointer-sign -Wno-error=shift-negative-value -Wno-address-of-packed-member -Wno-error=unused-but-set-variable -Wno-format-zero-length   -mno-aes -mno-avx  -std=iso9899:1999 -c /usr/src/sys/contrib/zlib/zutil.c -o zutil.o

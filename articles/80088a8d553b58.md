...
インストール後、`/etc/resolv.conf`を確認すると、
    ...
    *画面：twitteroauthをComposer経由でインストールした際のエラー*
    - 原因
        ...
        **不確実な情報となる為、正しい情報でない可能性あり**
        ...
        Composerコマンドの通信のみがエラーとなった為、試しにIPv6を無効化した結果、
        正常動作したという**事実のみで考察した原因**となる。
        :::
        ...
        - Composerコマンドのみ外部接続不可であった事
        - 契約中のプロバイダがIPv6に対応していない事

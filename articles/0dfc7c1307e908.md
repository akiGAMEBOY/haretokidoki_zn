...
function Write-IndentedOutput {
    ...
    param (
        ...
        # インデントに使用するインデックス数 ※デフォルトは1インデント（半角スペース4つ分）
    ...
    begin {
        ...
        # パイプラインで受け取ったオブジェクトを格納するコレクションを初期化する
        $collectedObjects = @()
    }

    process {
        # 各入力オブジェクトを配列に追加して蓄積する
...
```powershell:
# コマンドレット結果をパイプラインでFunctionに渡してインデントをあげる
Get-Process | Write-IndentedOutput -IndentCount 1

# コマンドレット結果のある変数を引数でFunctionに渡してインデントをあげる
$result = Get-Process
...
PS C:\Users\XXXX> Get-Process | Write-IndentedOutput -IndentCount 1
    ...
    Handles  NPM(K)    PM(K)      WS(K)     CPU(s)     Id  SI ProcessName                                               
    -------  ------    -----      -----     ------     --  -- -----------                                               
        133       8     1624       1988              4208   0 armsvc                                                        Handles  NPM(K)    PM(K)      WS(K)     CPU(s)     Id  SI ProcessName                                               

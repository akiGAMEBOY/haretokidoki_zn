...
- Outlook VBA で自動化プログラム（Subプロシージャ）を作成
  ...
  1. クイック アクセス ツール バー の登録
    デフォルトでは、すべてのフォルダーを送受信と元に戻すの2つが登録されていますが、追加でボタンを割り当てる事も可能。
    ...
    **注意事項**
    ...
    1. 作成したSubプロシージャを 追加(A)
      ...
      - 登録後のイメージ
        ...
        | 1個目 | Alt -> 1 | すべてのフォルダーを送受信 |
        | 2個目 | Alt -> 2 | 元に戻す |
        | 3個目 | Alt -> 3 | マクロ:メール新規作成<br> 👆 今回、追加 |
        | 4個目 | Alt -> 4 | マクロ:メール個別返信<br> 👆 今回、追加 |
        | 5個目 | Alt -> 5 | マクロ:メール全返信<br> 👆 今回、追加 |
        | 6個目 | Alt -> 6 | マクロ:メール転送<br> 👆 今回、追加 |
  1. Outlookでマクロを有効化
## 設定

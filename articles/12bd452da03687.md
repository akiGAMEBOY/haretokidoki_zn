...
　
そこで今回、自動化する方法を調べた結果、「 **Outlook VBA** × **クイック アクセス ツール バー** 」という組合せで、
複数アカウントでも自動化が実現できたので設定方法を紹介します。
...
タイトル通り、ここでの設定はOutlookの設定で**マクロを有効**とします。
　
これは悪意のある危険なコードを受け取った際、Outlook経由で実行される可能性がある為、
**Microsoftからは推奨されない設定**となります。

一般的な企業においても禁止されている設定となる為、取り扱いには十分ご注意ください。
:::
...
本項では、クイック アクセス ツール バーに自動化プログラムを登録しすべての設定が完了となります。
　
...
- 個別ウィンドウでの登録
  ...
  1. 作成したSubプロシージャを 追加(A)
      ...
      - メール転送用Subプロシージャ　　: Project1.ForwardEmailWithAddedToBCC
#### 登録後のイメージ
::::details 参考情報：初期設定のクイック アクセス ツール バー
:::message
**注意事項**

ボタンの並び順により割り当てられるショートカットキーが変化します。
:::
| ボタンの位置 | ショートカットキー | 初期設定<br>（メイン画面） | 初期設定<br>（個別ウィンドウ） |
| ---- | ---- | ---- | ---- |
...
### クイック アクセス ツール バー の利用方法
メイン画面のデフォルトでは、すべてのフォルダーを送受信と元に戻すの2つが登録され、
メールを個別に開いたウィンドウ（以降、個別ウィンドウ）では、上書き保存と元に戻す、やり直し、前のアイテム、次のアイテムの5つが登録されています。
...
-----
### 登録後のイメージ
  | ボタンの位置 | ショートカットキー | 追加後の設定<br>（メイン画面） | 追加後の設定<br>（個別ウィンドウ） |
  | ---- | ---- | ---- | ---- |
  | 1個目 | Alt -> 1 | すべてのフォルダーを送受信 | 上書き保存 |
  | 2個目 | Alt -> 2 | 元に戻す | 元に戻す |

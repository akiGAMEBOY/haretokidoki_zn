...
:::details 詳細情報：範囲選択で実行したコー（クリック・タップで折りたたみが開く）
...
その為、ここで対応した「 **出力先をターミナルに設定** 」はそのままの状態で引き続き作業してください。
...
- 私の環境の場合
    ...
    :::details 補足情報：なぜUTF-8 BOMで保存した方がよいの（クリック・タップで折りたたみが開く）
...
さらにCode Runnerで実行される一時ファイル「`tempCodeRunnerFile.ps1`」を確認すると文字コードが「**UTF-8**」である事が判明。

### 原因

日本語を含むマルチバイトを含まないPowerShellはUTF-8で動作するが、今回のコードでは日本語がコメント行やフォームの設定値などにあり、
BOM情報がファイルになく文字コードが判別できない為、PowerShellが自動的にShift-JISに変換し実行していた事が判明。

要約すると下記の流れでエラーが発生している事がわかった。
...
1. （PowerShellで自動）文字コードが判別できない日本語を含むps1ファイルとなる為 **Shift-JIS** で実行
1. **<< エラーが発生 >>**
...
1. 設定項目 `Code-runner: Executor Map` にあるリンク「 `settings.jsonで編集` 」をクリック
    ...
    :::details Code-runner: Executor Map の「 settings.jsonで編集 」による変化（クリック・タップで折りたたみが開く）

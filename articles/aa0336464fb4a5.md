...
Markdownファイル内にプログラムコードを書いた後、そのコードが正常に動くか検証する際、
わざわざ個別でファイルに書き出した後、実行して動作検証していました。

一度で動作検証が終われば良いですが試行錯誤すると、どうしても何度も繰り返し作業する必要があり、
面倒に感じていました。

ネットで探してみた結果、VS Codeの拡張機能である[Code Runner](https://marketplace.visualstudio.com/items?itemName=formulahendry.code-runner)を使用する事で、
複数行のコードを範囲選択した内容だけの実行が実現できるとの事で導入しました。
...
試しにアクティブファイルがMarkdownファイルの状態で実行していみると、`Code language not supported or defined.`というメッセージがあり、Markdownは実行形式のファイルではないので想定通り実行できませんでした。
...
Code Runnerでは初期設定されている主流の言語以外にも個別に追加で設定可能です。

- 設定ファイル（`settings.json`）内にファイルの拡張子（言語）毎の設定を追加する方法
...
```powershell:Code Runnerの選択範囲のコードを実行するもエラーが発生（ターミナルにてエラー）
...
結果は変わらず文字化けとエラーが発生しましたが、ここでの「 **出力先をターミナルに設定** 」と 後述する「 **Code RunnerでPowerShellの個別設定を追加** 」を組み合わせて対応しないと解決できない為、
ターミナルの設定はそのままとして後続の作業に移ります。

### 調査方法

ネットの情報を元に一般的な環境と私の環境で異なる部分を洗い出しました。

- 一般的な環境
    通常PowerShellスクリプトは文字コードがUTF-8の環境で動作する
- 私の環境
    マルチバイト（日本語）を含む文字列を取り扱う為、PowerShellスクリプトファイル（*.ps1）をUTF-8 BOM付きで保存している。
    :::details 補足情報：なぜUTF-8 BOMで保存した方がよいのか
    ...
    上記の回避策として日本語を含むPowerShellスクリプトは、文字コードを“UTF-8 BOM付き”で保存する事で文字化けやエラーがなく動作する。
...
調査結果をふまえてCode Runnerで実行される一時ファイル「`tempCodeRunnerFile.ps1`」の文字コードを確認すると文字コードが「**UTF-8**」だった。
前述した「調査方法 - 補足情報：なぜUTF-8 BOMで保存した方がよいのか」でも紹介している通り、通常のPowerShellはUTF-8で動作するが、
今回のコードでは、日本語がコメント行や文字列変数の値などにある為、PowerShellが自動的にShift-JISに変換し実行していた事が判明。

これらの情報から下記の流れでエラーが発生している事がわかった。

1. Markdownファイル内のコードを範囲選択してCode Runnerで実行
    範囲選択したコードには、日本語が含まれた状態
1. （Code Runnerで自動）で範囲選択部分を抜き出し **UTF-8 でps1ファイルを生成**
1. （Code Runnerで自動）生成したps1ファイルをPowerShellで実行
1. （PowerShellで自動）呼び出されたPowerShellでps1ファイルの中身を見ると日本語があり **Shift-JIS** で実行
1. **<< エラーが発生 >>**
    文字コード **UTF-8のps1ファイルをShift-JISとして実行してしまった為** 、日本語が文字化けしエラーが発生

### Code RunnerでPowerShellの個別設定を追加

Code Runnerの設定でPowerShellを実行する際に個別の処理を追加する。
具体的に追加する内容は下記の通り。

1. Code Runner実行時にターミナルで動作するよう変更（前述に記載した内容）
1. 実行時、一時的に作成されるファイルをUTF-8 BOM付きに変換
    ...
    なお、変換したファイルの保存先は一時フォルダ「 `$env:TEMP`（`C:\Users\ユーザー名\AppData\Local\Temp`） 」に保存する。
...
1. 設定項目 `Code-runner: Executor Map` にあるリンク「 `settings.jsonで編集` 」をクリック
    ...
    ```diff json:Code-runner: Executor Map の「 settings.jsonで編集 」による変化点
    {

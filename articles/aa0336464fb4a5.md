...
今回のCode Runner経由での実行では、拡張機能が実行する為の一時ファイルの文字コードがUTF-8（BOMなし）だった。

Code Runnerで実行した際、BOM情報がなくPowerShell側で文字コードが判別できなかった事と、
...
「 **出力先をターミナルに設定** 」と「 **Code RunnerでPowerShellの個別設定を追加** 」で、
文字コードをUTF-8 BOMのPowerShellのコードとしてCode Runner経由で実行できた。

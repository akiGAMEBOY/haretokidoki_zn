---
title: "ショートカットキーでプロセス再起動するPowerShellを実行する方法"
emoji: "💀"
...
最近使っているノートパソコンのメモリが16GBで、2026年現在さまざまなツールを併用している開発用途という観点だと決して多くはないメモリのサイズです。（最低は32GBぐらいほしい）

実際にメモリサイズを多く使用するアプリ（たとえば[Slack](https://ja.wikipedia.org/wiki/Slack_(%E3%82%BD%E3%83%95%E3%83%88%E3%82%A6%E3%82%A7%E3%82%A2))や[DB接続クライアントツール](https://ja.wikipedia.org/wiki/%E3%83%87%E3%83%BC%E3%82%BF%E3%83%99%E3%83%BC%E3%82%B9%E6%8E%A5%E7%B6%9A%E3%82%AF%E3%83%A9%E3%82%A4%E3%82%A2%E3%83%B3%E3%83%88)）を使用すると、頻繁に画面がフリーズすることでマウス操作やショートカットキーを受け付けなくなるケースが多くなってきました。

アプリの設定を最適化しても変わらず発生してしまうので、フリーズする度にタスクマネージャー経由でプロセスを終了するというオペレーションが面倒になってきました。
今回は**Windows標準機能だけ**を使ってショートカットキーでプロセスを再起動するPowerShellを作りました。

私と同じように、なるべくWindows標準のものを使って気軽にプロセス再起動したい方にオススメの記事です。
...
```mermaid
graph TD
    A["PowerShellスクリプト作成<br/>（引数でプロセスを指定し再起動）"]
    B["batファイル作成<br/>（PowerShellスクリプトの起動用）"]
    C["batファイルのショートカット作成"]
    D["ショートカットでショートカットキーを設定"]
...
1. 「`C:\Users\"ユーザー名”\OneDrive\ドキュメント\PowerShell\RestartProcess.ps1`」に文字コード**UTF-8 BOM付き**で保存
    PowerShellスクリプト内に日本語などマルチバイト文字がなく、すべて半角英数字の場合はUTF-8の保存（BOMなし）で問題ないです。
...
1. 「`C:\Users\"ユーザー名”\OneDrive\ドキュメント\PowerShell\bat\RestartProcess_Slack.bat`」に文字コード**SJIS**で保存
    PowerShellスクリプト内に日本語などマルチバイト文字がなく、すべて半角英数字の場合はUTF-8の保存（BOMなし）でも問題ないです。
    　

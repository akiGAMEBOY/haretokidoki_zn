...
# コピー先のファイルパス
$copiedFilePath = "D:\Downloads\DesignFileSample_copied.xlsx"

# ファイルをコピー
Copy-Item -Path $originalFilePath -Destination $copiedFilePath

# コピーしたファイルのプロパティを削除
$copiedFile = Get-Item -Path $copiedFilePath
$copiedFile.Attributes = 'Normal'

# すべてのプロパティを削除
$null = [System.IO.File]::SetAttributes($copiedFilePath, [System.IO.FileAttributes]::Normal)
$null = [System.IO.File]::SetCreationTime($copiedFilePath, (Get-Date))
$null = [System.IO.File]::SetLastAccessTime($copiedFilePath, (Get-Date))
$null = [System.IO.File]::SetLastWriteTime($copiedFilePath, (Get-Date))
...
Write-Host "ファイルのコピーとプロパティ削除が完了しました。"
```

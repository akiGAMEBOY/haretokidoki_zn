...
# DocumentFormat.OpenXmlをインストール
# （依存関係をスキップしてインストールする方法）
Install-Package -Name DocumentFormat.OpenXml -Source NuGet -Scope CurrentUser -SkipDependencies
...
function Remove-FileProperties {
    ...
    # Open XML SDKを使用してプロパティを削除
    [Reflection.Assembly]::LoadWithPartialName("DocumentFormat.OpenXml") | Out-Null
    [Reflection.Assembly]::LoadWithPartialName("System.IO.Packaging") | Out-Null

    $doc = [DocumentFormat.OpenXml.Packaging.SpreadsheetDocument]::Open($destinationPath, $true)
    $coreProps = $doc.PackageProperties

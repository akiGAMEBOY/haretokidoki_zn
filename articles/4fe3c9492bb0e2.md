...
動作できている環境は「**Adobe Acrobat XI Pro**」のみとなります。
...
[こちら](https://pdf-file.nnn2.com/?p=204)で紹介されている過去のバージョンに関しては、おそらく動作可能かと思います。
::::
...
![](https://storage.googleapis.com/zenn-user-upload/852c365d8681-20230404.png)
...
　
上記、入力データと同様、ルートを0階層とし9階層までを表示限度（階層00～階層09）としている。
#### 機能仕様
1. しおり読み込み
...
- しおりのジャンプをページ番号から座標にする方法【未検証】
    - 検索した文字列のページ番号や座標を返す処理
        ...
        どうしても、しおりで座標のジャンプをしたい場合は「[pdf_as](http://uchijyu.s601.xrea.com/wordpress/pdf_as/)」というアプリツールがあり、
        そのソフトではXMLファイルやテキストファイルを元に、しおりの登録が可能らしい。
        - PDF編集フリーソフト「pdf_as」
            http://uchijyu.s601.xrea.com/wordpress/pdf_as/
        - 「pdf_as」でテキストファイルを元にしおり登録
            https://freebsd.sing.ne.jp/tool/12/03/03.html
        - 他に「JPdfBookmarks」というツールでも可能らしい

...
PS C:\Windows\Logs>
```
### UNIX系“tail” = PowerShell“-Tail”
#### 最終桁からカウントし指定した行数分を表示
実行後はプロンプトが戻り通常通り次のコマンドが打てるように。
```powershell:コピー用
Get-Content StorGroupPolicy.log -Tail 20
```
```powershell:オプション「-Tail X」
...
PS C:\Windows\Logs>
```
#### 最終桁からカウントし指定した行数分＆リアルタイム表示
UNIXの“tail -f”とほぼ同じ動き。
```powershell:オプション「-Tail X -Wait」
...
（プロンプトが返ってこない。リアルタイム更新。Ctrl + Cでプロンプトに戻る）
```
#### 最終桁からカウントし指定した行数分＆リアルタイム表示＆Grep絞り込み
UNIXの“grep”とほぼ同じ動き。
```powershell:オプション「-Tail X -Wait | Select-String -Pattern X」
...
- 公式リファレンス「Get-Content」コマンド
    https://learn.microsoft.com/ja-jp/powershell/module/microsoft.powershell.management/get-content?view=powershell-7.3

...
```mermaid
flowchart TB
    A(["開始"]) --- B{"管理者として実行されているか確認"}
    B --> |"管理者として実行されている場合"| C["一時的に文字コードをUTF-8に変更"]
    B --> |"されていない場合"| X[["処理中断"]]
    C --- D["設定ファイルの読み込み"]
    D --- E["設定情報を表示（自動モードオンだと非表示）"]
    E --- F{"ツールを実行するか確認（自動モードオンだと非表示で自動でYes分岐へ）"}
    F --> |"Yes"| G["Windows Updateで使用するモジュールをインストール"]
    D --> |"No"| X
    G --- H["Windows Update の更新"]
    ※ 一括か個別か
    H --> |"実行結果：OK"| I["Microsoft Defender の更新"]
    H --> |"実行結果：NG"| X
    I --- J["アプリ（インストーラー版） の更新"]
    ※ 対応中
    J --> |"一括更新（自動的に設定を参照）"| H["自動で画面操作：テキスト形式で辞書ファイルを出力"]
    G --> |"個別更新（自動的に設定を参照）"| X[["処理中断"]]

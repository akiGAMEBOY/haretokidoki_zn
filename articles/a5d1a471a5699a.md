---
title: "Windows Update・Defender・アプリを一括アップデートする方法"
emoji: "🗂"
...
あらゆる自動更新機能をオフにしているWindows OS 環境でアップデートする場合、個々の管理画面に移動し、
更新処理する必要があり面倒に感じていました。
...
とインストーラー版アプリ、3つを一括でアップデートするPowerShellスクリプトを作成したので紹介します。
...
- 下記を手動で一括アップデートしたい方
    ...
    - インストーラー版 アプリ（source = winget）
        ※ 新しいMSIX形式のMicrosoft Store版 アプリ（UWP）には非対応
...
ただ、私のようにたくさんのアプリを導入し様々な用途に使用しているような複雑な方法でパソコンを運用している方であれば、
勝手に更新がかかるリスクが大きいと感じます。

## Microsoft Store版のアプリは

Store版のアプリも実現できそうな方法がいくつかの記事を見つけましたが、いずれの方法もうまく動作しませんでした。
...
ただ、Microsoft Store版のアプリも一括でアップデートする方法がないか調べましたが、見つけられませんでした。
ただ、今後のwingetコマンドのアップデートによりStore版アプリもコマンドで操作できるようになるらしいです。
...
[ユニバーサルWindowsプラットフォーム（UWP）](https://ja.wikipedia.org/wiki/ユニバーサルWindowsプラットフォーム)のアプリ（ソフトウェア）が配信されている環境。

統合されたフレームワークで開発されたアプリが配信されている為、使用するデバイス固有の問題など環境依存で発生する不具合が発生しにくいという利用者・開発者にともにメリットがある。
利用者のデメリットとしては、導入や更新でインターネットが必須となる為、クローズドの社内環境には向かない事。
一方、開発者のデメリットとしては、今まで提供していたインストーラー版アプリ（ソフトウェア）をUWPアプリ版で再開発（リプラットフォーム）する必要があり、通常の更新よりもコストがかかってしまう点が考えられる。

...
まず大前提に自動アップデートを設定してしまうと、
仮にトラブルが発生した場合に何を契機に発生したのか切り分けが難しくなってしまう。

Windows UpdateやMicrosoft Storeの両方で自動更新可能だが、意識して作業ができない。

ただ、手動でWindows Update と Microsoft Store をアップデートすると2つ画面を立ち上げて、
処理する必要があり面倒。

一括でアップデートできると作業効率化が図れる。
何か一括でアップデートする方法ができないか調べる。
...
<https://ja.wikipedia.org/wiki/Microsoft_Store>

[ユニバーサルWindowsプラットフォーム](https://ja.wikipedia.org/wiki/ユニバーサルWindowsプラットフォーム)のアプリ（ソフトウェア）が配信されている環境。
統合されたプラットフォームで開発されたアプリが配信されている為、使用しているデバイスの際などで発生する環境依存に起因するトラブルが発生しにくいメリットがある。
デメリットとしては今まで提供されていたソフトウェアをUWPアプリのプラットフォームで再開発（リプラットフォーム）する場合にコストがかかってしまう事だと思われる。

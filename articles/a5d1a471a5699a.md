...
1. システムやアプリの手動アップデートを一括で行いたい場合に使用
    ...
    おそらく、わざわざ手動でアップデートせずに、自動更新機能をオンにすると、解決できるのではと疑問に感じている方がいると思います。
    わたしなりに手動でアップデートしている理由があります。
    自動更新すると、大きく下記2つのデメリットがあると考えています。

    - 更新が起因となるトラブルが発生した際、切り分けが難しくなる可能性があること
    - 予期しないタイミングでネットワーク負荷がかかってしまうこと

    普段使用しているアプリ・ソフトウェアが少なくシンプルな構成でパソコンを使用している方は、
    自動更新で、これら問題は発生しにくいと思われます。
...
1. Microsoft Defender は、PowerShellのコマンドレット「Update-MpSignature」でアップデート
    ...
    標準機能ではなく、追加でソフトを導入している場合、Microsoft Defenderは自動で無効となる。

    ここで紹介するPowerShellスクリプトで、Microsoft Defenderのアップデートを無効にしたい場合、設定ファイル（`setup.ini`）を変更する必要あり。
    :::
...
1. インストーラー版アプリ は、Windowsのパッケージ管理システム「Windows Package Manager（winget）」でアップデート
    ...
    その為、ソフトウェア・アプリのアップデートに関しては限定的な対応となり、個別でアップデートが必要。
    今後のアップデート次第では、Store版アプリもアップデート対象となる可能性あり。

    :::details 補足情報：Microsoft Store版アプリ（UWP）も対応しようと検証したがNGだった < クリックで折りたたみが開く >
    何がしかの手法を使って[Microsoft Store](https://ja.wikipedia.org/wiki/Microsoft_Store)のアプリもアップデートできないか検証した。
    2024年4月現在、すべて正常動作しなかった。
    - モジュール「Appx」を使用した更新
    - winget 標準機能を使用した更新
    - winget 試験運用中の機能を使用した更新
        ...
        おそらく、以前あったStoreの設定「`experimentalMSStore`」の試験運用は終わったのかも？
...
:::details 補足情報：PowerShellの呼称について < クリックで折りたたみが開く >

Microsoftであるあるですが、PowerShellの正式名称がバージョンがあがるにつれてコロコロかわっています。
初期のコードネームでは、Monad。その後の正式名称がWindows PowerShellとなり、クロスプラットフォームとなったバージョン6.0の際には、PowerShell Core。その後、バージョン7.0では、PowerShell となりました。

たとえば、Microsoft Windows PowerShell というのが最初の正式名称でしたが、後に

...
- 下記を手動契機で一括アップデートしたい方
    ...
    - インストーラー版アプリ（source = winget）
        ※ MSIX形式のMicrosoft Store版 アプリ（[UWP](https://ja.wikipedia.org/wiki/ユニバーサルWindowsプラットフォーム)）には非対応

## 前提条件

1. システムやアプリの手動アップデートを一括で行いたい場合に使用
    :::details 補足情報：システムやアプリを手動でアップデートしたい理由
    なぜ自動更新機能をオンにしないのか。
    理由は単純で自動更新すると、大きく下記2つのデメリットがあると考えています。

    - 更新が起因となるトラブルが発生した際、切り分けが難しくなる場合があること
    - 予期しないタイミングでネットワーク負荷がかかってしまうこと

    普段使用しているアプリ・ソフトウェアが少なくシンプルな方法でパソコンを使用している方は、
    これら問題は発生しにくいと思います。

    :::

1. Windows Update は、PowerShellのモジュール「PSWindowsUpdate」を使用
    :::details 補足情報：そもそもWindows Updateって何をアップデートしているのか
...
1. Microsoft Defender は、PowerShellのコマンドレット「Update-MpSignature」を使用
    :::details ウイルス対策でMicrosoft Defender を使用していない場合
    Windows OS標準のウイルス対策ソフトは、Microsoft Defenderです。
    標準機能ではなく、追加でソフトを導入している場合、Microsoft Defenderは自動で無効になっています。

    このツールでMicrosoft Defenderのアップデートを無効にしたい場合は、設定ファイル（`setup.ini`）で設定を変更する必要あり。
    :::

1. インストーラー版アプリ は、Windowsのパッケージ管理システム「Windows Package Manager（winget）」を使用
    ::::details 補足情報：対象はソースが“winget”のアプリのみ
    winget listコマンドでインストールされているソフトウェア・アプリをみると項目「ソース」は“winget”と“msstore”、“（空欄）”の3種類が存在。
    
    2024年1月現在、検索するコマンド `winget search` や インストールするコマンド `winget install` については、Store版アプリも実行できますが、
    一括アップデートするコマンド [`winget upgrade`](https://learn.microsoft.com/ja-jp/windows/package-manager/winget/upgrade) では、ソースが“winget”のみが機能するもよう。

    その為、ソフトウェア・アプリのアップデートに関しては限定的な対応となる為、個別でアップデートが必要だと思われる。
    今後のアップデート次第では、Store版アプリもアップデート対象となる可能性あり。

    :::details メモ：Microsoft Store版アプリ（UWP）を対応させようとしたけどダメだった
    どうにかして[Microsoft Store](https://ja.wikipedia.org/wiki/Microsoft_Store)のアプリもアップデートできないか検証したが、検証した下記方法では正常に動作しなかった。
    - モジュール「Appx」を使用した更新
    - winget 標準機能を使用した更新
    - winget 試験運用中の機能を使用した更新
        ...
        おそらく、以前あったStoreの設定「experimentalMSStore」の試験運用は終わった？
        その試験の結果、設定ファイルを変更せずとも `winget search` や `winget install` などのコマンドでStore版のアプリが指定できるようになっているっぽい？
...
:::details PowerShellの呼称

★編集中

- Windows PowerShell
    バージョンの1.0 ～ 5.1 での正式名称は「Windows PowerShell」。
    - バージョン 1.0 ～ 5.0
        呼称はバージョン毎に「Windows PowerShell X.X」。
        3.0以降から\.NET Framework 4で動作するように。

    - バージョン 5.1
        呼称は「Windows PowerShell 5.1」。
        Windows 10 や Windows 11 で標準インストールされているバージョンとなる。
        バージョン5\.1ではDesktop版とCore版というエディションがある。
        Desktop Editionは、Windows環境で動作するPowerShell。Core Edtionは、Windows Server 2016 の インストールオプション、[Nano Server](https://atmarkit.itmedia.co.jp/ait/articles/1611/25/news051.html)で使用できる。

- PowerShell Core
    バージョン 6.0 ～ 6.2 での正式名称は「PowerShell Core」。
    - バージョン6.0 ～ 6.2
        バージョン6\.0からオープンソース化。
        また、ランタイムが\.NET Framework から \.NET Core になり、対応OSもmacOS（OS X）とLinuxで動作するようになった。

- PowerShell
    バージョン 7.0 ～ 7.4（2024.02現在）では正式名称が「PowerShell」となる。
    バージョンがあがるごとに\.NET Coreのバージョンがアップデートされる。
    バージョン 7.3においては、Windows 7 がサポート対象外となった。また、Test-Connectionコマンドレットの動作が意図的に変更されるなどの変化点もあった。
...
```powershell:winget uninstallコマンドを実行
...
```powershell:「$PSVersionTable」コマンド結果

...
1. インストーラー版アプリ は Windowsのパッケージ管理システム「Windows Package Manager（winget）」を使用
    ...
    一括アップデートするコマンド [`winget upgrade`](https://learn.microsoft.com/ja-jp/windows/package-manager/winget/upgrade) では、ソースが“winget”のみが対象となる。

    今後のアップデートによりStore版アプリも一括アップデート対象になる可能性はある。
    :::
    ...
    - winget 試験運用中の機能を使用した更新
        ...
        おそらく、以前あったStoreの設定「experimentalMSStore」の試験運用は終わった結果、`winget search` や `winget install` などのコマンドでStore版のアプリが指定できるようになったのだと思われます。

...
ただ、PowerShellのプロファイルは環境が変わる度に登録作業が必要となる為、面倒に感じていました。

この問題を解決するにあたり、今回は自作したFunctionを読み込み、PowerShellのプロファイルに自動で定義するコードを作成しました。
なお、自作したFunctionを読み込む方法は、ハードコーディングで読み込む方法 と 専用フォルダー配下のps1ファイルを読み込む方法 の2通りを準備しました。

## この記事のターゲット

- PowerShellユーザーの方
- 自作したFunctionを自動でPowerShellプロファイル（`$PROFILE`）に定義するコードを知りたい方

## コード

```powershell:実例（実際の自作Functionを定義したパターン）
...
if ($func_lines.Count -gt 0) {
    foreach ($line in $func_lines) {
        ...
        If (Select-String -Path $PROFILE -Pattern $name) {
            Write-Host '$ps_sourceで定義しているFunctionがプロファイルにありました。追記処理はスキップ。'
...
# 格納用フォルダー（自作Function）内のFunctionを$PROFILEに追記
$name = ''
$is_skip = $false
...
if ($func_lines.Count -gt 0) {
    foreach ($line in $func_lines) {
        $name = ($line -split ' ' | Select-Object -First 2)
        If (Select-String -Path $PROFILE -Pattern $name) {
            Write-Host '格納用フォルダー内のFunctionがプロファイルにありました。追記処理はスキップ。'
            Write-Host "（プロファイルに登録済みのFunction名: [$name]）"
            $is_skip = $true
            break
        }
...
:::details 実例（実際に私が作成したFunctionを定義してみる）

```powershell:実例（実際に私が作成したFunctionを定義してみる）
# PowerShellプロファイルのヘッダー
[System.String[]]$header = 
@"
#################################################################################
...
# ハードコーディングしてあるコード（$ps_source）を$PROFILEに追記
[System.String]$name = ''
[System.Boolean]$is_skip = $false
$func_lines = $ps_source -split "`n" | Where-Object { $_ -match "^Function " }
if ($func_lines.Count -gt 0) {
...
# 格納用フォルダー内のFunctionを$PROFILEに追記
$name = ''
$is_skip = $false
try {
    [System.String]$psfolder_source = (Get-Content "$psfolder_path\*.ps1" -Raw)
    ...
    Write-Host '格納用フォルダー内のps1ファイル読込み時にエラーが発生しました。追記処理はスキップ。'
    Write-Host "　格納用フォルダーに中身のあるps1ファイルが存在するか確認してください。"

---
title: "自作Functionを自動でPowerShellプロファイルに登録するコード"
emoji: "💳"
...
ただ、環境が変わる度にPowerShellのプロファイルを登録する必要があった為、面倒に感じていました。

この問題を解決するため、今回は自作したFunctionを読み込み、PowerShellのプロファイルに自動で定義するコードを作成しました。
なお、自作したFunctionを読み込む方法は、ハードコーディングしたコードを読み込む方法 と 専用フォルダー配下のps1ファイルを読み込む方法 の2通りあります。

## この記事のターゲット

- PowerShellユーザーの方
- 自作したFunctionを自動でPowerShellプロファイル（`$PROFILE`）に定義したい方
...
```powershell:実例（実際に私が作成したFunctionをハードコーディングした例）
# PowerShellプロファイルのヘッダー
[System.String[]]$header = 
@"
#################################################################################
...
専用フォルダー [D:\Documents\WindowsPowerShell\Microsoft.PowerShell_profile.ps1\..\user-defined-ps]  にps1ファイルがありません。処理を終了します。
PS C:\Users\"ユーザー名">
```

下記が実行後、ハードコーディングした自作Functionが反映されたプロファイルの内容。

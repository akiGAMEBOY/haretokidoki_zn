---
title: "自作Functionを自動でPowerShellプロファイルに定義するコード"
emoji: "💳"
...
## 自動でPowerShellプロファイルを作成し自作Functionを定義するコード
...
[System.String]$psfolder_path = (Convert-Path "$PROFILE\..\user-defined-ps")
...
"@ -split "`r`n"
    ...
    [System.String]$pyfolder_path = (Convert-Path "$PROFILE\..\user-defined-py")
...
Function Get-PyEncoding {
    ...
    # 存在チェック
    if (-Not(Test-Path $targetfile)) {
        Write-Host "[$fullpath_targetfile] does not exist." -ForegroundColor Red
        return
    }

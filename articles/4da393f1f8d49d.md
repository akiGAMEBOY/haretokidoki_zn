...
前述している通り、ハードコーディングしたFunction と 専用フォルダー配下にあるps1ファイル内のFunction の2つから読み込み、PowerShellのプロファイルに定義します。
...
:::details 実例（実際に私が作成したFunctionで作成したコード）

```powershell:実例（実際に私が作成したFunctionで作成したコード）
# PowerShellプロファイルのヘッダー
[System.String[]]$header = 
@"
#################################################################################

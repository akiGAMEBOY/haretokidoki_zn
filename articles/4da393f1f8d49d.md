...
if (-Not(Test-Path "$PROFILE\..\user-defined-func")) {
    # プロファイルのPowerShellスクリプトが格納されている場所にフォルダーを作成
    New-Item -Path "$PROFILE\..\user-defined-func" -ItemType 'directory' -Force
}
...
Add-Content -Path $PROFILE -Value (Get-ChildItem "$PROFILE\..\user-defined-func\*.ps1")
...
Get-ChildItem "$PROFILE\..\user-defined-func\*.ps1" | %{.$_}

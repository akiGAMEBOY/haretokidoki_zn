...
- **課題1**：オプション「`/F`」をつけないとファイルが対象にならない
    作成したFunctionでは規定でフォルダーもファイルもすべてリストアップする作りに。
- **課題2**：フォルダー と ファイル を見分けにくい
    作成したFunctionではフォルダーに小なり（`<`）と大なり（`>`）で囲むように。
    （例：`< フォルダー名 >`）
- **課題3**：オブジェクト数が多いとコマンド出力後、クリップボードへのコピーしにくい
    オプションで出力結果をクリップボードにコピーするように。
- **課題4**：文字のみの表示となるため、視認しにくい
...
function Write-TreeView {
    ...
    function Write-FolderTree {
        ...
        foreach ($item in $items) {
            ...
            if ($item.PSIsContainer) {
                # フォルダーの場合
                $returnString += "$indent$line< $item >$($newlineTable[$NewlineCode])"
                $returnString += (Write-FolderTree -CurrentPath $item.FullName -Levels $Levels)
            } else {
                # ファイルの場合
                $returnString += "$indent$line$item$($newlineTable[$NewlineCode])"
            }
            ...
            if ($isLastItem) {
                $returnString += "$indent$($newlineTable[$NewlineCode])"
            }
    ...
    $Levels = @{}
    $returnString +=  "$Path$($newlineTable[$NewlineCode])"
    $returnString +=( Write-FolderTree -CurrentPath $Path -Levels $Levels)

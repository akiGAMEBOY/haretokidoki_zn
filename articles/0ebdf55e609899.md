...
一方、UnixやLinuxのコマンド（UnixやLinuxのプロセス確認コマンドは`ps`）と比較すると、PowerShellのコマンドレットは長い文字列で構成されたコマンドとなります。

CLIでコマンドレットを入力すると一回いっかい長い文字列のコマンドを打つ必要がある為、時間がかかり面倒。
そんなケースの時、短い文字列のコマンド、エイリアス（別名）を活用する事で時間につながります。

今回は以前から気になっていたPowerShellコマンドレットを調べ表形式で紹介。
なお、確認するPowerShellのバージョンはWindows標準で導入されているバージョン`5.1`と2023年8月現在、最新バージョンである`7.3.6`を対象に確認しています。

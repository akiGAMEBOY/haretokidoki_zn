...
一方、UnixやLinuxのコマンド（UnixやLinuxのプロセス確認コマンドは`ps`）と比較するとPowerShellのコマンドレットは長い文字列で構成されたコマンドであると言えます。

PowerShellのCLIでコマンドレットを入力しようとすると不便ですが、このようなケースでコマンドのエイリアス（別名）が便利です。

今回は以前から気になっていたPowerShellコマンドレットを調べ表形式で紹介します。
なお、確認するPowerShellのバージョンはWindows標準で実装されているバージョン`5.1`と2023年8月現在、最新バージョンである`7.3.6`を対象に確認しています。

## この記事のターゲット
- PowerShellユーザーの方
- 下記の環境下のコマンドレットのエイリアスを把握したい方
...
例えば、Get-Aliasコマンドのエイリアスは「 `gal` 」との結果ですが、「 `alias` 」とコマンドを実行しても、
Get-Aliasやgalと同じ結果が表示されます。
:::
### コマンド結果の生データ（省略あり）
...
:::details 全てのコマンド結果を表示したエイリアスの対応表
...
v5.1 と v7.3.6 の2つのバージョンで確認しました。
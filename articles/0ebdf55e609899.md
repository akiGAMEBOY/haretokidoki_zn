---
title: "PowerShell“5.1”と“7.3”コマンドレットのエイリアス一覧"
emoji: "🦔"
...
---
## 概要
PowerShellのコマンドレットは、通常「 `Get-` 」などの文字から始まるコマンド（例えばプロセスを確認する`Get-Process`）があり、
UnixやLinuxのコマンド（UnixやLinuxだと`ps`）と比較すると長めのコマンドです。
ですが、この長いコマンドは省略可能、つまりエイリアスが登録されています。

今回はWindowsにおいて標準で実装されているPowerShellのバージョン5.1と2023年8月現在、最新バージョン7.3.6で、
エイリアスを確認してみました。

## この記事のターゲット
- PowerShellユーザーの方
- Windows標準のPowerShell と 最新のPowerShell 環境のエイリアス名を把握したい方

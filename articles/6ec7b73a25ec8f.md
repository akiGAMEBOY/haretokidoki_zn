...
買いたてホヤホヤのWindows OSのパソコン。コマンドで ~~Microsoft Edgeをアンインストール~~（👈使用により不可でした） してから **Google Chromeをインストール** しちゃおう！
...
★
https://zenn.dev/baker_link_lab/articles/2ea4260daf32e4
...
### 応用技術：このFunctionを実行するとパッケージを検索してID指定でインストール可能

下記をコピーしてWindows PowerShell ウィンドウに貼り付けるだけで、名前もしくはIDでパッケージを検索しIDとバージョンを指定してインストールが可能です。

```powershell:パッケージを検索してID指定でインストール可能
...
下記のように引数を指定すると入力待ちはスキップされてすべて自動で進む。

```powerhsell
Install-WingetPackage -PackageId "microsoft.powershell" -Version "7.5.0.0"
```

:::details “これを実行すると名前を検索してID指定でインストール可能”のソース全体

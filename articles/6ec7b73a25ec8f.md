...
実際にバージョンアップ版のPowerShellを使ってみみると、処理時間が短縮したり機能が強化されたりと、
...
Function Install-WingetPackage {
    ...
    if (-Not $Version) {
        ...
        Write-Host "最新版 '$Version' をインストールします。" -ForegroundColor Yellow
    } elseif (-Not ($availableVersions -contains $Version)) {
        Write-Error "指定されたバージョン '$Version' は利用可能なリストにありません。"
        $statusCode = -23
        return $statusCode
    }

    Write-Host "$PackageId のバージョン $Version をインストールします..." -ForegroundColor Cyan

    # 標準出力を破棄してインストール処理を実行
    (winget install --id $PackageId --version $Version --silent | Out-Null)
    $wingetExitcode = $LASTEXITCODE

    if ($wingetExitcode -eq 0) {
        Write-Host "$PackageId バージョン $Version のインストールが完了しました。" -ForegroundColor Green
    } else {

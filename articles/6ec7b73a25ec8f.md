---
title: "[Windows]wingetコマンドでソフトウェア・アプリをインストール（PowerShell x winget）"
emoji: "🔮"
...
ユーザーアカウント制御があるので、完全自動化とは言えないものの簡単にインストールできるようになります。

知る人ぞ知るWindows PowerShellを使い`winget`コマンドで「Google Chrome」を導入する方法を例に紹介。

## シナリオ

買いたてホヤホヤのWindows OSのパソコン。
不要なMicrosoft Edgeをアンインストールして**Google Chromeのインストール**をすべてコマンドでやっちゃおう！

## 対応手順

1. Windows PowerShell ウィンドウを開く
    :::details Windows PowerShell ウィンドウ の起動方法 < クリックで折りたたみが開く >

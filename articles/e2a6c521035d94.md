...
Windows OSでPowerShellの自動化ツールを作成する際、環境変数を編集できた方が利便性が高いと感じ調べました。
...
:::details 実際のコマンド結果（Get-ChildItem Env:） <クリックで折りたたみが開く>
...
新規で作成した際は`New-Item`を使用していましたが、変更する場合は`Set-Item`で対応可能です。

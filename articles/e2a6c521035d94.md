...
下記の通りGet-Itemではパスの終端のファイルもしくはフォルダーの情報を表示。

```powershell:Get-Itemコマンドレット
...
下記の通りGet-ChildItemでパスの終端がフォルダーの場合は配下の一覧を表示。
なお、パスの終端がファイルの場合、Get-Itemコマンドレットと同じ結果になる。

```powershell:Get-ChildItemコマンドレット

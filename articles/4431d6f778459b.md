...
1. つづいてもインストール中の応答に「 `y` 」で返すことによりImportExcelをインストール
...
- PowerShell Gallery よりモジュールデータをダウンロード
    [こちらのページ](https://github.com/dfinke/ImportExcel/releases/)より `対象のバージョン（vX.X.X）` → `Source code (zip)` をダウンロードする。

    :::details `v7.8.9` の場合

    1. Releases ページ → v7.8.9 → Assets → [Source code (zip)](https://github.com/dfinke/ImportExcel/archive/refs/tags/v7.8.9.zip) をクリックしダウンロード

        `ImportExcel-7.8.9.zip` という名前でダウンロードされる。
    
    1. フォルダー構成を下記のとおり変更

        ```poewrshell:変更前のフォルダー構成
        ImportExcel-7.8.9
        │  .gitattributes
        │  .gitignore
        │  Add-Subtotals.ps1

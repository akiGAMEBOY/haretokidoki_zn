...
:::details システムフォルダーなどによりOSユーザーにアクセス権がない場合は、エラーが発生。
...
エラーが発生しないように`GetChildItem`コマンドレットのオプションで`-ErrorAction SilentlyContinue`でエラーを飛ばし続行する動きにすると解決。

:::

### 指定したフォルダー配下の空フォルダーを一覧で出力

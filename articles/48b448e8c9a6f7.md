...
直前のコマンド結果（終了コードを指定した場合は、そのコード）が`%ERRORLEVEL%`に反映される。
#### 直接コマンドを実行した場合
```:OKパターンの戻り値を確認
...
```
```:NGパターンの戻り値を確認
PS C:\WINDOWS\system32> Not-Exits-Command
Not-Exits-Command : 用語 'Not-Exits-Command' は、コマンドレット、関数、スクリプト ファイル、または操作可能なプログラム
の名前として認識されません。名前が正しく記述されていることを確認し、パスが含まれている場合はそのパスが正しいことを確認
してから、再試行してください。
発生場所 行:1 文字:1
+ Not-Exits-Command
+ ~~~~~~~~~~~~~~~~~
    + CategoryInfo          : ObjectNotFound: (Not-Exits-Command:String) [], CommandNotFoundException

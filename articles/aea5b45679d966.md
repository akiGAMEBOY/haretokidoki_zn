...
PythonスクリプトはPowerShellプロファイル（$PROFILE）と同じ階層に専用のフォルダーを作成し、
...
なお、PythonスクリプトのソースコードはPowerShell Function内に定数の位置づけで定義。
Pythonのコードを変更したい場合、Function内のコード修正し、なおかつ保存済みの `chardet_runner.py` を手動削除が必要です。
...
- 意外と簡単にPythonスクリプトを実行できたので、今後違う使用用途のPythonスクリプトも作成してみたい！

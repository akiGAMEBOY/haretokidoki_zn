...
なお、PythonスクリプトのソースコードはPowerShell Function内に定数として定義。
実行するPythonのコードを変更したい場合、Function内のコード修正し、なおかつ保存済みの `chardet_runner.py` を手動で削除する必要があります。
...
- 意外と簡単にPowerShellからPythonスクリプトを実行できたので、今後あらゆる用途でPythonスクリプトを作成したい！

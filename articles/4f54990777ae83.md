...
$\mathrm{Processブロックを使った関数} \neq \mathrm{foreachを使った関数}$
...
- `Process`ブロックを使った関数の場合（**真のストリーム処理**）
...
関数の利用者が、その場の状況に応じて`-ErrorAction`パラメーターを渡すだけで、「停止（`Stop`）」「続行（`Continue`）」「無視（`Stop`）」といった挙動を自由に選択できます。これにより、関数が非常に柔軟で再利用性の高いツールになります。

では、呼び出し側でエラーが発生した時に処理を中断するようにしてみます。

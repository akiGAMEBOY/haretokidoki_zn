...
「`Process`ブロックを使った関数」と「`foreach`を使った関数」の動きの違いを見ることで、本質的なストリーム処理についてみていきましょう。

##### `Process`ブロックを使った関数の場合（本質的なストリーム処理）

**特徴**: データが1つ来るたびに、`Process`ブロックがその都度実行されます。**メモリには常に一度に1つのデータしかロードされないため、巨大なデータセットでも効率良く処理**できます。
...
**特徴**: **すべてのデータを一度にメモリ上に展開してから**処理を開始。そのため、**扱うデータセットが非常に大きい場合（ファイルサイズが数GBのデータなど）は、メモリを大量に消費する可能性**があります。
...
function Get-ServiceStatusOnServers {
    ...
    param(
        [Parameter(Mandatory=$true, ValueFromPipeline=$true, HelpMessage="サービス状態を確認するコンピューターのホスト名またはIPアドレスを指定します。")]
        [string]$ComputerName,

        [Parameter(HelpMessage="確認するサービス名を指定します。デフォルトは 'BITS' です。")]
        [string]$ServiceName = "BITS",

        [Parameter(HelpMessage="結果をCSVファイルに出力する場合のパスを指定します。")]
        [string]$OutputPath,

        [Parameter(HelpMessage="リモート接続に使用する資格情報を指定します。ワークグループ環境などで必要になります。")]
    ...
    Begin {
        ...
        if ($OutputPath) {
            ...
            if (-not (Test-Path -Path $parentDir)) {
                ...
                # CSV出力はスキップされることを示すフラグ
                $script:SkipCsvOutput = $true
            } else {
                $script:SkipCsvOutput = $false
            }
        } else {
            $script:SkipCsvOutput = $false
        }
    ...
    Process {
        ...
        $statusObject = $null
        try {
            # Invoke-Command を使ってリモートで Get-Service を実行
            # -Credential が指定されていればそれを使用
            $remoteService = Invoke-Command -ComputerName $ComputerName -ScriptBlock {
                param($serviceNameToGet)
                Get-Service -Name $serviceNameToGet -ErrorAction Stop
            } -ArgumentList $ServiceName -Credential $Credential -ErrorAction Stop
            ...
            if ($remoteService) {
                ...
                # サービスの発見に失敗したがエラーとして捕捉されなかった場合（稀だが念のため）
                $statusObject = [PSCustomObject]@{
                    Computer = $ComputerName
                    Service  = $ServiceName
                    Status   = "NotFound"
                    Message  = "エラー: サービス '$ServiceName' がリモートコンピューター '$ComputerName' で見つかりませんでした。"
                }
            }
        }
        catch {
            ...
            $statusObject = [PSCustomObject]@{
                Computer = $ComputerName
                Service  = $ServiceName
                Status   = "Error" # "Unknown" よりも "Error" の方が状況を表しやすい
                Message  = "エラー: リモートコンピューター '$ComputerName' への接続またはサービス取得に失敗しました。詳細: $errorMessage"
            }
        }
    ...
    End {
        ...
        if ($results.Count -gt 0) {
            if ($OutputPath -and -not $script:SkipCsvOutput) {
                try {
                    ...
                    # CSV出力に失敗した場合の代替処理（フォールバック）
                    Write-Warning "CSVファイルへの出力に失敗しました。エラー: $($_.Exception.Message)"
                    Write-Host "`n"
                    Write-Host "--- CSV出力が失敗したため、結果をコンソールに表示します ---"
                    $results | Format-Table
                }
            } elseif ($OutputPath -and $script:SkipCsvOutput) {
                Write-Warning "出力パスが指定されましたが、親ディレクトリが存在しないためCSV出力はスキップされました。"
                $results | Format-Table # コンソールには表示する
            }
            else {
            ...
            Write-Warning "処理されたコンピューターはありませんでした。"
        }
...
:::message
**実行前に「`Enable-PSRemoting`」でリモート接続を有効にする必要あり！**

この関数ではLAN内のWindows端末に`Invoke-Command`を使ってリモートアクセスして、`Get-Service`でサービスの状態を確認します。
これを実現するためには接続元と接続先で「`Enable-PSRemoting`」でお互いのリモート接続を有効にしておく必要があります。

お試して実行した場合は、以前に投稿している[こちらの記事](https://zenn.dev/haretokidoki/articles/fd1eeb92fa4095#%E4%BA%8B%E5%89%8D%E8%A8%AD%E5%AE%9A)をご参照し、**リモート接続を有効**にしてください。

:::

```powershell:通常の実行イメージ
PS C:\Users\XXXX> $serverList | Get-ServiceStatusOnServers -ServiceName "TermService" -OutputPath "D:\Downloads\ServiceStatus_TermService.csv" -Verbose
VERBOSE: [Begin] 処理を開始します。
VERBOSE: [Process] サーバー 'localhost' を処理中...
...
VERBOSE: [Process] サーバー '127.0.0.1' を処理中...
...
VERBOSE: [Process] サーバー '8.8.8.8' を処理中...
...
VERBOSE: [Process] サーバー 'Dev-WinOS' を処理中...
...
VERBOSE: [Process] サーバー '169.254.130.39' を処理中...
...
- **エラーで即停止（ErrorAction:`Stop`）**
    ...
    VERBOSE: [Begin] 処理を開始します。
    Get-ServiceStatusOnServers: 出力先の親ディレクトリが存在しません: X:\NO_EXISTS_FOLDER
    PS D:\Downloads>
    ```
...
- **初期値の動作（ErrorAction:`Continue`）**
    ...
    VERBOSE: [End] 最終処理を開始します。
    WARNING: 出力パスが指定されましたが、親ディレクトリが存在しないためCSV出力はスキップされました。

    Computer       Service     Status  Message
    --------       -------     ------  -------
    localhost      TermService Running 成功
    127.0.0.1      TermService Running 成功
    8.8.8.8        TermService Error   エラー: リモートコンピューター '8.8.8.8' への接続またはサービス取得に失敗しました。…
    Dev-WinOS      TermService Running 成功
    169.254.130.39 TermService Running 成功

...
function Get-ServiceStatusOnServers {
    [CmdletBinding()] # これにより -ErrorAction, -Verbose などの共通パラメータが使えるようになる
    param(
        # パイプラインから複数のサーバー名を配列として受け取れるように変更
        [Parameter(ValueFromPipeline=$true)]
        [string[]]$ServerLists,
    ...
    Process {
        ...
        foreach ($computerName in $ServerLists) {
        ...
        if ($results.Count -gt 0) {
            if ($OutputPath) {
                try {
                    ...
                    Write-Host "`n"
                    Write-Host "--- 何らかの要因でCSV出力が失敗したため、結果をコンソールに表示します ---"
...
- **エラーで即停止（ErrorAction:`Stop`）**
    ...
    詳細: [Process] サーバー 'Dev-Python' を処理中...
...
```powershell
    # --- 実行例 ---
    # サーバーリスト
    $serverList = "localhost", "127.0.0.1", "8.8.8.8", "Dev-Python", "169.254.130.39"

    # 関数を実行し、結果をC:\temp\ServiceStatus.csvに出力
    $serverList | Get-ServiceStatusOnServers -ServiceName "TermService" -OutputPath "D:\Downloads\ServiceStatus_TermService.csv" -Verbose -ErrorAction Stop
    ```

    ```powershell:実際の実行結果
    PS D:\Downloads> $serverList | Get-ServiceStatusOnServers -ServiceName "TermService" -OutputPath "D:\Downloads\ServiceStatus_TermService.csv" -Verbose
    詳細: サーバーのサービス状態のチェックを開始します...
    詳細: サーバー 'localhost' を処理中...
    詳細: サーバー '127.0.0.1' を処理中...
    詳細: サーバー '8.8.8.8' を処理中...
    詳細: サーバー 'Dev-WinOS' を処理中...
    詳細: サーバー '169.254.130.39' を処理中...
    ✅ 結果をCSVファイルに出力しました: D:\Downloads\ServiceStatus_TermService.csv
    詳細: すべての処理が完了しました。
    PS D:\Downloads>
    ```
...
- **`Begin`ブロック**
    最初の1度だけ、全サーバーの結果をまとめるための「空のリスト」を準備しています。
- **`Process`ブロック**
    サーバー1台ずつに接続を試み、その結果を整形して`$results`リストにため込みます。。
- **`End`ブロック**
    最後に一度だけ、まとまった結果をCSVファイルに出力するため、`Export-Csv`を使って書き出します。
...
| :--- | :--- | :--- |
| **Background Intelligent Transfer Service** | `BITS` | **【ネットワーク/安定性】** Windows Updateなどで使用される、ネットワークのアイドル帯域を利用してファイルを賢く転送するサービス。これが停止していると、Windows Updateなどが失敗する原因になります。 |
| **Windows Update** | `wuauserv` | **【セキュリティ/安定性】** Windowsのセキュリティ更新プログラムや機能更新を管理する最重要サービス。これが停止していると、PCが脆弱な状態のままになります。BITSと密接に関連しています。 |
| **DNS Client** | `Dnscache` | **【ネットワーク】** Webサイトのドメイン名をIPアドレスに変換した結果を一時保存（キャッシュ）します。「インターネットに繋がらない」「特定のサイトが開けない」といった問題の際に確認します。 |
| **DHCP Client** | `Dhcp` | **【ネットワーク】** ルーターなどからIPアドレスを自動的に取得・管理します。これが停止していると、ネットワークに接続できなくなります（IPアドレスが取得できないため）。 |
| **Print Spooler** | `Spooler` | **【基本機能】** 印刷ジョブを管理します。「印刷ができない」「印刷キューからジョブが消えない」といった印刷関連のトラブルの際に、まず確認・再起動する対象です。 |
| **Windows Time** | `W32Time` | **【安定性/セキュリティ】** コンピュータの時刻をインターネット上の時刻サーバーと同期させます。時刻のズレは、Webサイトの証明書エラーや、特に企業環境での認証エラーの原因となります。 |
| **Remote Procedure Call (RPC)** | `RpcSs` | **【システム基盤】** 多くのWindowsサービスやアプリケーションが互いに通信するための基盤となる、非常に重要なサービスです。これが停止すると、Windows自体が正常に動作しなくなります。依存関係が極めて多いのが特徴です。 |
| **Windows Audio** | `AudioSrv` | **【基本機能】** Windowsの音声機能を管理します。「PCから音が出ない」という問題の際に確認します。`AudioEndpointBuilder`という関連サービスも重要です。 |
| **Windows Defender Firewall** | `MpsSvc` | **【セキュリティ/ネットワーク】** Windows標準のファイアウォール機能です。「特定のアプリケーションが通信できない」「ネットワーク共有にアクセスできない」といった問題が、この設定に起因することがあります。 |
| **Remote Desktop Services** | `TermService` | **【リモート接続】** 他のコンピューターからリモートデスクトップ接続を受け付けるために必要です。「リモートデスクトップで接続できない」という場合に確認します。 |
| **Cryptographic Services** | `CryptSvc` | **【セキュリティ/安定性】** ファイルの署名検証や証明書の管理など、Windowsのセキュリティの根幹を支えるサービスの一つです。Windows Updateが正常に動作するためにも必要です。 |
...
# Get-Serviceでリストにあるサービスの状態をまとめて取得し、コンピュータ名も表示
Get-Service -Name $serviceList -ComputerName localhost | Select-Object Status, Name, DisplayName, MachineName
```
:::

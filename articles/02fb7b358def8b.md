---
title: "[Windows]消せない回復パーティションの削除方法 - DISKPARTコマンド"
emoji: "💿"
...
ディスクの管理 ではなく **≪DISKPARTコマンド≫ を使用し削除**できたので、
その手順を紹介します。
...
間違えが無いよう **DISKPARTコマンドの結果** と **ディスクの管理** を、
...
## 雑学：Windowsのシステムドライブは、なぜ「C」から始まっているのか
...
### 答え
HDDが一般化した際、**既にフロッピーディスクドライブ（FDD）でAドライブとBドライブを使っていた為**となります。
...
時代とともにフロッピーディスクは、より大容量なCDやDVDに移り変わった結果、
CドライブはHDDで使用されている状態のまま、**AドライブとBドライブだけが使用されなくなった**。
というのが、ざっくりした経緯です。

### 最近の移り変わりは...
最近ではHDDがより高速に読み書きでいるSSDとなったり、
外部ストレージとして使用されていたCDやDVD、USBメモリ、外付けHDD、NASなどもオンラインストレージに移り変わったりしています。

今後は、メガネや家の窓、車のガラスなどにモニタが付属するようになり、
端末を保有するというのが古くなってくるんですかね。今後の進化が楽しみです。
### 参考記事

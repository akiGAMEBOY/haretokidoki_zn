...
PowerShellでは「Function内での標準出力が、戻り値としてFunctionの呼び出し元に渡される」という**仕様**があります。

要は『 **Function内で標準出力により意図しない戻り値になるよ** 』ということ。
...
## 「Function内で標準出力により意図しない戻り値になる」とは
...
上記のコードでは、呼び出したFunction内で標準出力を伴うコマンドをいくつか実行しています。

### “仕様で戻り値が期待値とならない”コードを実行した結果
...
Function（`New-Folder`）を実行した戻り値が、System.Int32 ではなく 「**System.Object[]**」で返ってきたことがわかります。
...
:::details デバッグメッセージを追加したコード（全体）

デバッグ関連のコードを追加したコードです。
...
Function（`New-Folder`）を実行する前後で `$statusCode` の結果を確認すると、データ型は `System.Int32` で 中身は数字 `0` のままという事がわかります。

このFunctionでは、数値型の `$statusCode` を返す想定なのですが、実際にはFunction内で `New-Item` や `Write-Output` など標準出力の伴うコマンドを実行しているため、
数値

さらに `Cannot convert the "System.Object[]" value of type "System.Object[]" to type "System.Int32".` というメッセージは、
Function（`New-Folder`）の戻り値がPowerShell特有の仕様により `System.Object[]` で返りそのデータを数値型の `$statusCode` に代入しようとしたため、発生したエラー。
ここで行っているデータ型の変換もPowerShellの仕様で、別のデータ型に代入すると自動でキャスト（型の変換）されるというものです。

結果、`System.Object[]` から `System.Int32` はキャスト（変換）できないデータ型同士なので `Cannot convert` から始まる **変換エラーメッセージが発生した** という流れ。

もう少し言葉を変えて説明すると本来、Function（`New-Folder`）で期待していたデータ型は、「`System.Int32`」。しかし、Function内の標準出力により「`@("New-Itemの標準出力結果", "Write-Outputの標準出力結果", "$statusCodeの数値")`」というような `System.Object[]` の戻り値が返ってきたということになります。

では、実際に戻ってきた `System.Object[]` の中身が想定通りか確認してみましょう。

### 本当に戻り値が System.Object[] で値も標準出力が入っているか確認

実際にFunction（`New-Folder`）の戻り値内を確認してみます。
確認するために新たにコードを追加！

```diff powershell:戻り値のデータ型と中身をチェックするコード
# メイン処理
-[int]$statusCode = 0
+[object]$resultData = @()
...
:::details 戻り値のデータ型と中身をチェックするコード（全体）
...
```powershell:これは「New-Itemコマンドレットの標準出力」
New-Folder: フォルダーを新規作成。
New-Folderが正常終了。statusCode[0]
```

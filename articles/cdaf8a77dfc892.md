...
Windows Server を代表とする Windows OS で取り扱われる改行コードは、CRLF（`\r\n`）です。また、RedHat などの UNIX系サーバー の規定では、LF（`\n`）だったりします。
...
今回、自作したPowerShellのFunctionを使用すると比較的、効率良く改行コードの変換が可能になると思います。
...
テキストファイル内にある改行コードを任意のコードに変換可能。このFunctionをPowerShellスクリプトに組み込みジョブスケジューリングすることで効率的に変換できると思います。
...
ただ今回、組織のニーズに応じて対応できる範囲を増やせてよかったと感じました。

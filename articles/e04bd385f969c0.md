...
誰もその理由や経緯を知らないようだが、システムハンガリアンと呼ばれるようになったものをWindowsチームのドキュメントライター達が、うっかり発明してしまったようだ。

誰かが、どこでシモニーの論文を読み、そこで彼が“型（type）”という言葉を使ってしまったので、クラスまたは型システム、コンパイラがする方チェックのような、型（type）を意味するのだと思ってしまった。彼の意図はそうではなかった。彼は“タイプ（type）”という言葉が何を意味するのか、非常に注意深く正確に説明したが、意味を成さなかった。甚大な被害となった。

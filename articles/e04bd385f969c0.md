...
published: false
---
以前より元記事をご存じの方や再三に渡って議論してきたと感じている方などは、いまさら何を英訳してるんだ。という感想になる方がいらっしゃると思いますが、
私は元記事やハンガリアン記法にシステムハンガリアンやアプリケーションハンガリアンがあったという事を最近、知りました。

頭にすんなり入ってくるような英訳の記事が見つからなかったので、自分なりに英訳し学習した結果を共有できればと思い記事にして紹介します。

なお、本記事で登場するAmazonに関連した書籍のリンクは、すべてAmazonアソシエイト[^1]のリンクとなります。
...
- ハンガリアン記法を（誤った解釈で）悪いと決めつけている方

:::message
**読む前の注意事項**

- 私は英語のスペシャリストではありません。
- 誤った翻訳内容が含まれている可能性があります。
- 機械翻訳の使用 と 雰囲気で翻訳しています。
    基本的に機械翻訳（[DeepL翻訳](https://www.deepl.com/translator)）を使いニュアンスを汲み取り雰囲気で翻訳しました。
    高い確率で表現やニュアンスに間違えがあると思われます。
- 原文とは異なる見出し構成です
    翻訳内容を記載 → 原文を表示 → 翻訳内容を記載 → ... というような内容で繰り返す構成上、
    日本語に翻訳した内容と原文を極力、1画面に収めたかったので、この記事では独自の見出しを追加しました。
...
## 例題（問題を見つけやすくする為のコードを意図的に作り込むという事）
...
システムハンガリアンは、Windowsプログラミング・ドキュメントの標準となり、Windowsプログラミング学習のバイブルとなった[チャールズ・ペゾルドのプログラミングWindows](http://www.charlespetzold.com/pw5/)のような書籍（[書籍のリンク](https://www.amazon.co.jp/dp/482229496X)）で広まり、たちまちハンガリアン記法の主流となった、マイクロソフト社内でさえも、WordとExcelを除いたチームで自分たちの間違いを理解しているプログラマーは、極めて少なかった。
★リンク張り替え

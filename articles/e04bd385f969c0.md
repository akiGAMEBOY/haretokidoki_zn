---
title: "[日本語訳]Making Wrong Code Look Wrong（間違ったコードを間違ったように見せる）- Joel Spolsky氏"
emoji: "🍣"
type: "idea" # tech: 技術記事 / idea: アイデア
topics: ["blog", "翻訳"]
published: false
---
以前から[元記事](https://www.joelonsoftware.com/2005/05/11/making-wrong-code-look-wrong/)をご存じの方や再三に渡り議論した方などは、何をいまさらという話題だと思いますが、
私は元記事やハンガリアン記法にシステムハンガリアンやアプリケーションハンガリアンというものがあったという事を最近、知りました。
...
- 翻訳した内容の取り扱いにはご注意ください。
    翻訳方法は、機械翻訳による結果をベースに私個人のアバウトな文章校正を行いました。
    詳細を説明すると、基本的に機械翻訳（[DeepL翻訳](https://www.deepl.com/translator)）を使いニュアンスを汲み取った上で、
    文章を校正しています。
    前述のとおり英語のスペシャリストではなく、どちらかというと不得意な私が雰囲気で翻訳しています。
    高い確率で誤訳があると思いますので翻訳内容の取り扱いにはご注意ください。
- 原文とは異なる見出し構成です。
    日本語に翻訳した内容と原文を極力、1画面に収めたかったので、「翻訳内容を記載 → 原文を表示 → 翻訳内容を記載 → ...」というように内容を繰り返す記事の構成で、
    翻訳した内容が小間切れとなっています。その為、利便上、原文にはない独自の見出しを追加しています。

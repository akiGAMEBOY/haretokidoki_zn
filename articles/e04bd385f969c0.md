...
:::
## 私はハンガリー
さて、悪名高いハンガリー記法の話に戻そう。

ハンガリー記法はマイクロソフトのプログラマー、[チャールズ・シモニイ（Charles Simonyi）](https://ja.wikipedia.org/wiki/チャールズ・シモニー)によって考案された。シモニーがマイクロソフトで手掛けた主要プロジェクトのひとつが、ワードだった。実際、彼はゼロックス・パルク（Xerox PARC）でブラボー（Bravo）という世界初の[WYSIWYG](https://ja.wikipedia.org/wiki/WYSIWYG)ワープロを作るプロジェクトを指揮した。

WYSIWYGワープロでは、スクロール可能なウィンドウがあるため、すべての座標をウィンドウからの相対座標かページからの相対座標かで解釈が必要となる。

シモニイがハンガリー記法と呼ばれるものを使い始めたのは、それが多くの理由の内の一つだと、私は推測している。その記法はハンガリー語のように見え、シモニイはハンガリー出身だったので、この名前が付いた。シモニイ版のハンガリー記法では、すべての変数の頭に、その変数が含む種類（kind）を示す小文字のタグが付けられていた。

...
HTMLエンコードされた文字列や、既知の安全な場所から来た文字列はすべて、プレフィックス（接頭辞）“s”（安全な文字列）で始まる名前の変数に格納しなければならない。
...
新しい規約で注目してほしいのは、安全でない文字列でミスを犯した場合、コーディング規約が守られている限り、とある1行のコードでで簡単に見つける事ができるということ。
★ここから

---
title: "WebPファイルが静止画像かアニメーション画像かプログラムで判定する方法"
emoji: "💭"
...
WebPファイルにある チャンク（Chunk）ヘッダー情報 で “静止画像” か “アニメーション画像” か判定する方法が見つかりました。
...
### 詳細情報：WebPファイルのヘッダー情報

一般的なバイナリーエディターのように1行あたり16バイトで表現しています。
...
![アニメーションのWebPファイルをバイナリーエディターで確認](https://storage.googleapis.com/zenn-user-upload/b7655a59dc06-20240105.png)
...
なお、PowerShellでは「`Format-Hex`」コマンドレットを使用しバイナリー情報を表示する事が可能。
...
変換が正しくできているという前提があれば、わざわざバイナリーでヘッダー情報を確認しなくとも拡張子が「webp」か判定した方がはやい。
...
- バイナリーモードでXMP拡張データにあるチャンクデータで「 アニメーション画像 」か判断
    先頭からByteでカウントすると「31バイト目 ～ 34バイト目 のデータ」が`ANIM`
    1行16バイトで表現すると「“2行目 15桁目 ～ 16桁目”と“3行目 1桁目 ～ 2桁目”を結合した文字列」が`ANIM`
- 上記以外の場合、「 静止画像 」と判断

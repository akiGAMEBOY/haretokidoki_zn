...
高い圧縮率を実現するため情報が切り捨てられている為なのか、どうやら判定する方法はなさそう。
...
- WebPファイル の種類で 静止画 か GIFアニメ か判断したい方
- プログラムで判断できる可能な限りWebPファイル の ヘッダー情報 で判断できる
...
上記のようにわざわざヘッダー情報を確認しなくとも拡張子が「webp」か判定した方が速い。

#### 拡張データがない静止画か判定

```powershell:IF文で拡張データがない静止画か判定
[System.Object[]]$webp_hex_ascii = (Format-Hex -Path .\screentogif_multi-frame_lossress.webp | Select-Object Ascii)
...
上記の通り、プログラムではしっかりと判定ができないと思われる。
かっこ悪い方法だが、該当ファイルをプレビューして作業者に静止画 か GIFアニメ なのか判定してもらう方がよさそう。
...
- プログラムで拡張データがない静止画か判定する方法
    WebPファイルをバイナリモードで読み取りヘッダー情報で判断
    1. WebPファイル形式か判断
        1桁目 ～ 4桁目 and 9桁目 ～ 12桁目 が 'RIFF' and 'WEBP' の場合、WebPファイル形式と判断。
    1. 拡張データがない静止画か判断
        13桁目 ～ 16桁目 が 'VP8X'以外の場合、WebPファイルが拡張データが含まれていない静止画と判断。

...
本来、実装する上では不要なコードとなる為、ここで紹介する際は最適化したコード。
...
function CheckPortMultipleServers {
    param (
        [Parameter(Mandatory=$true)][System.String[][]]$ServerPortLists
    )
    $TempStrings=New-Object System.Text.StringBuilder

    for($i = 0; $i -lt $ServerPortLists.Length; $i++) {
	    $TargetRow = $ServerPortLists[$i]
        
        try {
            Test-NetConnection $TargetRow[0] -Port $TargetRow[1]
        }
        catch {
            $TempStrings=New-Object System.Text.StringBuilder
            @("Test-NetConnectionコマンドレットでエラーが発生しました。`r`n",`
              "エラーとなったコマンド：[Test-NetConnection $($TargetRow[0]) -Port $($TargetRow[1])]")|
            ForEach-Object{[void]$TempStrings.Append($_)}
            [System.String]$Messages = $TempStrings.ToString()
...
# Function 実行
[System.String[][]]$ServerPortLists = @(
    @('localhost','8080'),
    @('localhost','8000'),
    @('google.com','443'),
    @('smtp.google.com','25')
)
CheckPortMultipleServers $ServerPortLists
```

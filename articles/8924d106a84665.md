...
また、`Write-`ではじまるコマンドレットは、他にもあり使用用途が理解できていなかったこと。
...
1. Write-EventLog
    ...
    既存のイベントソース（イベントプロバイダー）やイベントIDを調べる方法を紹介。また、自作のイベントソースを作る方法にも触れています。
    　
    また、6.0以降の環境下において自作のイベントソースで出力したい場合、`New-WinEvent`では対応できなかった為、違う方法で検証しました。
    　
    詳しくは、下記の記事をご参照ください。

...
各コマンドレットの使用用途を調べた内容を表形式でまとめました。
...
1. Write-EventLog
    ...
    既存のイベントソース（イベントプロバイダー）やイベントIDを調べる方法を紹介。また、自作のイベントソースを作る方法も紹介。
    自作のイベントソースでイベントログを出力したい場合、6.0以降の`New-WinEvent`では対応できなかった為、違う方法で検証しました。
    詳しくは、下記の記事をご参照ください。

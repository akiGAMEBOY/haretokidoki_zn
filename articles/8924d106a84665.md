...
| --- | --- | --- | --- |
| Write-Output | パイプラインにオブジェクトを送信する<br>PowerShell 2.0で導入 | `Write-Output 'Success Stream Message.' 1> Redirect01.txt` |
| Write-Error | エラーメッセージを表示する<br>PowerShell 2.0で導入 | `Write-Error 'Error Stream Message.' 2> Redirect02.txt` |
| Write-Warning | 警告メッセージを表示する<br>PowerShell 2.0で導入 | `Write-Warning 'Warning Stream Message.' 3> Redirect03.txt` |
| Write-Verbose | 詳細メッセージを表示する<br>PowerShell 2.0で導入 | `Write-Verbose 'Verbose Stream Message.' 4> Redirect04.txt` |
| Write-Debug | デバッグメッセージを表示する<br>PowerShell 2.0で導入 | `Write-Debug 'Debug Stream Message.' 5> Redirect05.txt` |
| Write-Information | 情報メッセージを表示する<br>PowerShell 5.0で導入 | `Write-Information 'Information Stream Message.' 6> Redirect06.txt` |
| Write-Progress | プログレスバーを表示する<br>PowerShell 2.0で導入 | ※ Progressストリームへのリダイレクトは不可 |
| Write-Host | コンソールにメッセージを表示する<br>PowerShell 5.0以降からInformationストリームとして出力されるように。 | 5.0以降の場合：`Write-Host 'Information Stream Message.' 6> Redirect06.txt`  |
...
どのストリームにも属していなかったWrite-Hostですが、PowerShell 5.0以降から **Information ストリーム** に属するようになりリダイレクトできるようになりました。
また、同じくPowerShell 5.0から導入された、Write-Informationを使用することでも **Information ストリーム** に出力、かつリダイレクト可能。

それぞれのコマンドレットで Information ストリーム への出力は可能ですが、すぐには違いがわかりませんでした。
そこで、Write-Host と Write-Information の違いを調べました。
結論を書くと、

- Write-Hostは、コンソール表示に特化した出力コマンド
    （上記のWrite-Outputとの違いもふまえると、標準出力せずにコンソール表示に特化した出力コマンドとなる。）
- Write-Informationは、ログを記録することに特化した出力コマンド
...
文字から出力するエラーレベルが変わるというのは、理解できますが、具体的な動作の違いを調べました。

結論を書くと、

- Write-Errorは、続行不可能な異常が発生した場合の出力コマンド
- Write-Warningは、続行不可まではいかないが警告すべき異常が発生した場合の出力コマンド
...
1. Write-Output
    Successストリームに出力するためのコマンドレット。一般的なIT用語にすると標準出力のこと。

    主にパイプラインに渡したり、変数に代入したりする目的で使用される。
    
    なお、このコマンドレットをFunction内で使用し戻り値を別で設定した場合、期待通りの戻り値とならない。
    たとえば、Function内で `Write-Output 'Test Message'`と文字列（`System.String`）を指定し、`return $True`と論理値（`System.Boolean` または `bool`）で指定した場合の戻り値は、`System.Object[]`として返ってくる。
...
1. Write-Error
    Error ストリームに出力するためのコマンドレット。一般的なIT用語にすると標準エラーのこと。
...
1. Write-Verbose
    ...
    また、コマンドレットにパラメーター `-Verbose` を指定する方法でもデバッグ出力が可能。
    このパラメーターは `$VerbosePreference` の設定にかかわらず出力可能。
...
1. Write-Debug
    ...
    このパラメーターは `$DebugPreference` の設定にかかわらず出力可能。
...
1. Write-Information
    Informationストリームに出力するためのコマンドレット。
...
1. Write-Host
    ...
    この出力は、パイプラインで送ることができないが、メッセージの文字色などの見た目が変更できる。
    ユーザーインターフェイスに重点を置いたコマンドレット。

    なお、Write-Information との違いは、文字色・背景色など見た目を変更できるのがWrite-Host。Informationストリーム上でタグ付けできるなどの記録ができるのがWrite-Information。
...
1. Write-EventLog
    このコマンドレットは、ストリームではなくイベントビューアーに出力するログが生成可能です。

    `Write-EvnetLog`コマンドレットは、PowerShell 5.1まで実行可能ですが、
    6.0以降のCoreエディションでは、実行できません。かわりにCoreエディションでは、`New-WinEvnet`というコマンドレットで対応します。

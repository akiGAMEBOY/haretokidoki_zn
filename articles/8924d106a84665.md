...
- Writeではじまる他の出力系コマンドレットの違いも理解したい方
...
| --- | --- | --- |
| Write-Output | パイプラインにオブジェクトを送信<br>PowerShell 2.0で導入 | `Write-Output 'Success Stream Message.' 1> Redirect01.txt` |
| Write-Error | エラーメッセージを表示<br>PowerShell 2.0で導入 | `Write-Error 'Error Stream Message.' 2> Redirect02.txt` |
| Write-Warning | 警告メッセージを表示<br>PowerShell 2.0で導入 | `Write-Warning 'Warning Stream Message.' 3> Redirect03.txt` |
| Write-Verbose | 詳細メッセージを表示<br>PowerShell 2.0で導入 | `Write-Verbose 'Verbose Stream Message.' 4> Redirect04.txt` |
| Write-Debug | デバッグメッセージを表示<br>PowerShell 2.0で導入 | `Write-Debug 'Debug Stream Message.' 5> Redirect05.txt` |
| Write-Information | 情報メッセージを表示<br>PowerShell 5.0で導入 | `Write-Information 'Information Stream Message.' 6> Redirect06.txt` |
| Write-Progress | プログレスバーを表示<br>PowerShell 2.0で導入 | ※ Progressストリームへのリダイレクトは不可 |
| Write-Host | コンソールにメッセージを表示<br>PowerShell 5.0以降からInformationストリームとして出力されるように。 | 5.0以降の場合：`Write-Host 'Information Stream Message.' 6> Redirect06.txt`  |
| Write-EventLog | イベントログを出力 | - |
...
どのストリームにも属していなかった `Write-Host` ですが、PowerShell 5.0以降から **Information ストリーム** に属するようになりリダイレクトできるようになりました。
...
そこで、`Write-Host` と `Write-Information` の違いを調べると、
...
文字から出力するエラーレベルが変わるというのは、理解できますが、具体的な動作の違いを調べると、
...
ざっくり書くと2024年4月現在、PowerShellには出力する場所（Output Streams）が合計7種類あります。
今回紹介するコマンドレットの大半は、その7種類それぞれに出力するためのコマンド。
...
1. Write-Output
    **Success ストリーム** に出力するためのコマンドレット。一般的なIT用語にすると標準出力のこと。
    ...
    前述の説明では、標準出力とコンソール表示を行うと記載しているが、厳密にいうと機能は、標準出力のみで、
    PowerShellウィンドウが標準出力された内容をコンソール表示している。
...
1. Write-Host
    ...
    PowerShell 5.0以降では **Information ストリーム** に出力されるようになった。

    5.0より前のバージョンでは、どのストリームにも出力できなかったため、パイプラインで送ることもできなかった。
...
1. Write-EventLog
    このコマンドレットは、ストリームは関係なくイベントビューアーに出力するログが生成可能。

    `Write-EvnetLog`コマンドレットは、PowerShell 5.1まで実行可能だが、
    6.0以降のCoreエディションでは、実行できない。
    かわりに6.0以降のCoreエディションでは、`New-WinEvnet`というコマンドレットで対応。

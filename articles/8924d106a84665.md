...
また、`Write-`ではじまるコマンドレットが他にもあり使用用途がわからなかったこと。

大きくこの2点を解決するため、「Write-」からはじまるコマンドレットについて調べてみました。
...
| Write-Progress | プログレスバーを表示<br>PowerShell 2.0で導入 | ※ Progressストリームのリダイレクトは不可 |
...
それぞれのコマンドレットで Information ストリーム への出力は可能ですが、違いが理解できませんでした。
...
1. Write-Output
    **Success ストリーム** に出力するためのコマンドレット。一般的なIT用語にすると標準出力のこと。
    ...
    このオブジェクト内に標準出力した文字列と戻り値で設定した論理値の2つが入ってしまう。
...
1. Write-EventLog
    ...
    :::details 関連記事：PowerShellでイベントログを出力する方法をより深く調べた < クリックで折りたたみが開く >

...
| コマンドレット | 説明 | 導入された時期 | リダイレクト方法の例 |
| --- | --- | --- | --- |
| Write-Output | パイプラインにオブジェクトを送信する | PowerShell 2.0 | `Write-Output 'Success Stream Message.' 1> Redirect01.txt` |
| Write-Error | エラーメッセージを表示する | PowerShell 2.0 | `Write-Error 'Error Stream Message.' 2> Redirect02.txt` |
| Write-Warning | 警告メッセージを表示する | PowerShell 2.0 | `Write-Warning 'Warning Stream Message.' 3> Redirect03.txt` |
| Write-Verbose | 詳細メッセージを表示する | PowerShell 2.0 | `Write-Verbose 'Verbose Stream Message.' 4> Redirect04.txt` |
| Write-Debug | デバッグメッセージを表示する | PowerShell 2.0 | `Write-Debug 'Debug Stream Message.' 5> Redirect05.txt` |
| Write-Information | 情報メッセージを表示する | PowerShell 5.0 | `Write-Information 'Information Stream Message.' 6> Redirect06.txt` |
| Write-Progress | プログレスバーを表示する | PowerShell 2.0 | ※ Progressストリームへのリダイレクトは不可 |
...
1. Write-Information
    ...
    ログファイルなどに記録するために用いられるとのこと。
    なお、Write-Host との違いは、パイプラインに渡せるか、文字色・背景色など見た目を変更できるか。という違いがある。

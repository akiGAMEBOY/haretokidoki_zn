...
ファイルサイズが大きいファイルの絶対パスが長い場合、右側の項目「ファイルサイズ」や「最終更新日時」が見切れてしまう。

![](C:\Users\Uenishi%20Takaaki\AppData\Roaming\marktext\images\2025-05-01-08-56-58-image.png)

そのような場合は、ウィンドウサイズを横方向に広げてからFunctionを実行する必要がある

![](C:\Users\Uenishi%20Takaaki\AppData\Roaming\marktext\images\2025-05-01-08-59-39-image.png)
...
パイプラインでOut-GridViewに渡すと

```powershell
PS C:\Users\XXXX> Get-LargeFiles -DriveLetter C -TopCount 20 | Out-GridView
```

![](C:\Users\Uenishi%20Takaaki\AppData\Roaming\marktext\images\2025-05-01-09-07-59-image.png)
...
`-NoTypeInformation`は、CSVファイルの先頭に記録される`#TYPE Selected.System.IO.FileInfo`を消すために指定。

他にもエンコードの指定やXXXなどもあるので、詳細はExrpot-Csvコマンドレットの公式記事を参照。

...
[過去の記事](./49dacc3516c441#調査方法)でもCドライブが枯渇した際にファイルサイズの大きいデータを調べ、その方法を記載しました。今回は関数化（Function）してみました。

「[PowerShell + ファイルサイズ + 大きい](https://www.google.com/search?q=PowerShell+ファイルサイズ+大きい)」などで検索すると、すでに数多くの記事が投稿されています。

当初、わたしが記事を作成する意味がないと思っていましたが、他の方が対応していない下記2点の対応を行ってみました。
...
このような場合場合は、ウィンドウサイズを横方向に広げてからFunctionを実行すると、下記のようにすべて表示可能。
...
```csv:出力した「D:\Downloads\get-largefiles.csv」の中身
"Rank","FullName","Size","LastWriteTime"
"1","C:\ESD\Windows\sources\install.esd","4117938608","2024/03/13 8:46:18"
"2","C:\Program Files (x86)\Adobe\Acrobat 11.0\Setup Files\{AC76BA86-1033-FFFF-7760-000000000006}\Data1.cab","638279605","2012/09/24 13:02:32"
"3","C:\ESD\Windows\sources\boot.wim","495620879","2024/03/13 8:46:08"

...
:::message
**コマンド結果の変数をコンソール出力する場合の注意**

下記では変数にいれた結果を`Write-Output`を使い表示させています。
かりに`Write-Output`ではなく`Write-Host`で表示すると、コマンド結果の改行コードが正しく解釈されません。

`Write-Host`であると本来、複数行ある`winget upgrade`のコマンド結果が1行のみで表示されてしまうのでご注意を。
:::
...
もし、この直接コマンドを実行した結果で文字化けしている場合は本記事、解決方法の前にPowerShellウィンドウの設定を見直す必要があります。
規定値で設定されているフォントは日本語対応されていないフォントなので、`BIZ UDゴシック` や `ＭＳ ゴシック` に変更する必要があると思われます。
[こちら](https://jpwinsup.github.io/blog/2021/08/19/UserInterfaceAndApps/PowerShell/PowerShell-Console-characters-garbled-for-CJK-languages/)の公式ページで解説されていますので、ご参考にどうぞ。

```powershell:6.0以降の環境で直接コマンドを実行（5.1でも同じ結果）
...
:::details 推測・想像の見解：変数代入 → 変数出力 → 文字化け の流れ
...
ただ、同じ `Shift JIS` にもかかわらず、“**直接コマンドを実行したケースでは、文字化けしなかった**”というケースに関しては、どのようなロジックで正常に出力できているのか把握できていません。
...
:::details 実際のコマンド結果（コンソール出力の文字コードをUTF-8に変更）
...
:::detials 実際のコマンド結果（文字コード設定の一括変更）
...
文字コードを変更した後、設定内容をタイトルに表示させたい方は、下記を参考に導入してください。
